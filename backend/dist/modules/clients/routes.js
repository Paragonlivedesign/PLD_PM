...
function encodeCursor(c) {
    return Buffer.from(JSON.stringify({ u: c.updated_at, id: c.id }), "utf8").toString("base64url");
}
...
clientsRouter.get("/", requirePermission("clients:read"), asyncHandler(async (req, res) => {
    try {
        const q = listClientsQuerySchema.parse(req.query);
        const limit = q.limit ?? 25;
        const dec = decodeCursor(q.cursor);
        const rows = await repo.listClients(pool, req.ctx.tenantId, q.search, limit + 1, dec?.u ?? null, dec?.id ?? null);
...
clientsRouter.get("/:id", requirePermission("clients:read"), asyncHandler(async (req, res) => {
...
clientsRouter.put("/:id", requirePermission("clients:update"), asyncHandler(async (req, res) => {
    try {
        ...
        const updated = await repo.updateClientRow(pool, req.ctx.tenantId, routeParam(req.params.id), patch, expected);
        if (!updated && expected) {
...
clientsRouter.delete("/:id", requirePermission("clients:delete"), asyncHandler(async (req, res) => {
    const n = await repo.countEventsForClient(pool, req.ctx.tenantId, routeParam(req.params.id));
    if (n > 0) {
