...
export async function getDepartments(tenantId, options) {
...
export async function getTenantForApi() {
    ...
    const base = resolveTenantSettings((row.settings ?? {}), row.name);
    if (full) {
        return {
            id: row.id,
            name: row.name,
...
export async function updateTenantForApi(body) {
    const ctx = getContext();
    const row = await tenantRepo.findTenantById(pool, ctx.tenantId);
    if (!row)
        throw new HttpError(404, "NOT_FOUND", "Tenant not found");
    const prevName = row.name;
    const prevSettings = { ...row.settings };
    let name = row.name;
    ...
    const changed = [];
    if (body.name !== undefined && name !== prevName)
        changed.push("name");
    if (body.settings !== undefined)
...
function parseIsActiveFilter(q) {
...
export async function listDepartmentsApi(query) {
    const ctx = getContext();
    const include = query.include_counts === "true" || query.include_counts === "1";
...
export async function createDepartmentApi(body) {
    const ctx = getContext();
    ...
    if (head_id) {
        ...
        if (!hp)
            throw new HttpError(400, "VALIDATION", "head_id not found", "head_id");
    }
    ...
    if (Number.isNaN(sortOrder))
        throw new HttpError(400, "VALIDATION", "Invalid sort_order", "sort_order");
...
export async function updateDepartmentApi(id, body) {
    const ctx = getContext();
    ...
    if (body.head_id !== undefined)
        patch.head_id = body.head_id;
...
export async function reorderDepartmentsApi(orderedIds) {
    const ctx = getContext();
