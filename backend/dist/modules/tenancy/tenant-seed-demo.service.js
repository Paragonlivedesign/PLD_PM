...
export function isTenantDemoSeedAllowed() {
    if (process.env.NODE_ENV !== "production")
        return true;
    const v = process.env.PLD_ALLOW_TENANT_DEMO_SEED;
    return v === "1" || v === "true";
}
function resolveSeedDemoCatalogUrl() {
    ...
    throw new HttpError(500, "INTERNAL", "Could not find scripts/seed-demo-catalog.mjs (deploy the repo scripts/ folder or run the API from the monorepo root).");
}
export async function seedTenantDemoOperationalData(tenantId) {
    if (!isTenantDemoSeedAllowed()) {
        throw new HttpError(403, "FORBIDDEN", "Demo seed is disabled. Set PLD_ALLOW_TENANT_DEMO_SEED=1 in production or use a non-production NODE_ENV.");
    }
    const mod = await import(resolveSeedDemoCatalogUrl());
    const seedDemoCatalog = mod.seedDemoCatalog;
