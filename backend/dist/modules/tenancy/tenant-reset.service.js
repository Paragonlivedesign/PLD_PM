...
export function isTenantDataResetAllowed() {
    if (process.env.NODE_ENV !== "production")
        return true;
    const v = process.env.PLD_ALLOW_TENANT_DATA_RESET;
    return v === "1" || v === "true";
}
async function deleteDocumentFiles(client, tenantId) {
    const r = await client.query(`SELECT storage_key FROM documents WHERE tenant_id = $1`, [tenantId]);
 ...
 */
export async function resetTenantOperationalData(tenantId) {
    if (!isTenantDataResetAllowed()) {
        throw new HttpError(403, "FORBIDDEN", "Tenant data reset is disabled. Set PLD_ALLOW_TENANT_DATA_RESET=1 in production or use a non-production NODE_ENV.");
    }
    ...
    try {
        ...
        await run(`DELETE FROM scheduling_conflict_participants WHERE conflict_id IN (SELECT id FROM scheduling_conflicts WHERE tenant_id = $1)`);
