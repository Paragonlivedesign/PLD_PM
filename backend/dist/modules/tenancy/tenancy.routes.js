...
tenantRouter.get("/", asyncHandler(async (_req, res) => {
...
departmentsRouter.post("/", requireAnyPermission("tenancy.departments.create", "departments:create"), asyncHandler(async (req, res) => {
    const data = await svc.createDepartmentApi(req.body);
    res.status(201).json(ok(data));
}));
departmentsRouter.patch("/reorder", requireAnyPermission("tenancy.departments.edit", "departments:update"), asyncHandler(async (req, res) => {
    const body = req.body;
...
departmentsRouter.get("/:id", asyncHandler(async (req, res) => {
...
departmentsRouter.put("/:id", requireAnyPermission("tenancy.departments.edit", "departments:update"), asyncHandler(async (req, res) => {
    const id = paramId(req.params.id);
    if (!UUID_RE.test(id)) {
        res.status(400).json({
            data: null,
    ...
    const data = await svc.updateDepartmentApi(id, req.body);
    res.status(200).json(ok(data));
}));
departmentsRouter.delete("/:id", requireAnyPermission("tenancy.departments.delete", "departments:delete"), asyncHandler(async (req, res) => {
    const id = paramId(req.params.id);
    if (!UUID_RE.test(id)) {
        res.status(400).json({
            data: null,
