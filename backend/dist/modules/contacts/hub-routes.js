...
import { listContactPersonsForHub, } from "./person-repository.js";
import { writeAuditLog } from "../audit/service.js";
function filterHubByPermissions(rows, permissions) {
    const has = (p) => permissions.has("*") || permissions.has(p);
    const canReadMembership = (pt) => {
        if (pt === "client_organization")
            return has("clients:read");
        if (pt === "venue")
            return has("venues:read");
        if (pt === "vendor_organization")
            return has("vendors:read");
        return false;
    };
    return rows
        .map((row) => ({
...
contactPersonsHubRouter.get("/", requireAnyPermission("clients:read", "venues:read", "vendors:read"), asyncHandler(async (req, res) => {
...
contactPersonsHubRouter.get("/export", requireAnyPermission("clients:read", "venues:read", "vendors:read"), asyncHandler(async (req, res) => {
    const ctx = tryGetContext();
    if (!ctx) {
        res.status(500).json(singleError("INTERNAL", "Missing context", 500).body);
        return;
    ...
    for (const p of filtered) {
        const primary = p.emails.find((e) => e.is_primary)?.address ??
            p.emails[0]?.address ??
            "";
...
contactPersonsHubRouter.post("/import/dry-run", requireAnyPermission("clients:update", "venues:update", "vendors:update"), asyncHandler(async (_req, res) => {
    res.json(ok({ staged: [], conflicts: [] }));
}));
contactPersonsHubRouter.patch("/bulk", requireAnyPermission("*"), asyncHandler(async (_req, res) => {
    res.status(501).json({
        data: null,
        meta: null,
        errors: [
