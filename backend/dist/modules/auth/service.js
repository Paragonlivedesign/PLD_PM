...
function forgotRateKey(tenantId, email) {
    return `${tenantId}:${email.toLowerCase()}`;
}
function checkForgotRate(tenantId, email) {
...
function genOpaqueToken() {
    return randomBytes(32).toString("base64url");
}
export function toUserProfileResponse(user, permissions) {
...
export async function resolvePermissionsForUser(tenantId, userId, roleId) {
...
async function issueTokens(user) {
    ...
    await repo.insertRefreshToken(pool, uuidv7(), user.tenant_id, user.id, refreshHash, familyId, exp);
    return {
        access_token: access,
...
export async function register(body) {
    ...
    if (!roleId) {
        throw new HttpError(503, "SERVICE_UNAVAILABLE", "Tenant is missing a viewer role; cannot register");
    }
    ...
    if (!user)
        throw new HttpError(500, "INTERNAL", "Failed to create user");
...
export async function login(body) {
    ...
    if (user.locked_until && user.locked_until > new Date()) {
        throw new HttpError(423, "LOCKED", `Account locked until ${user.locked_until.toISOString()}`, "locked_until");
    }
    ...
    const permissions = await resolvePermissionsForUser(refreshed.tenant_id, refreshed.id, refreshed.role_id);
...
export async function refresh(body) {
    ...
    const exp = new Date();
    exp.setDate(exp.getDate() + REFRESH_DAYS);
    await repo.insertRefreshToken(pool, uuidv7(), row.tenant_id, row.user_id, newHash, row.family_id, exp);
    const access = await jwt.signAccessToken({
        sub: user.id,
        tid: user.tenant_id,
        role: user.role_name,
...
export async function logoutAllSessions(tenantId, userId) {
    await repo.revokeAllUserRefreshTokens(pool, userId);
    await cache.invalidateUserPermissions(tenantId, userId);
    return { message: "All sessions logged out" };
}
export async function revokeUserSessionsAdmin(tenantId, targetUserId) {
...
export async function getMe(tenantId, userId) {
...
export async function updateMe(tenantId, userId, body) {
    const user = await repo.findUserById(pool, tenantId, userId);
    if (!user)
        throw new HttpError(401, "UNAUTHORIZED", "Unauthenticated");
    const patch = {};
    if (body.first_name !== undefined)
        patch.first_name = String(body.first_name);
    if (body.last_name !== undefined)
        patch.last_name = String(body.last_name);
    if (body.phone !== undefined) {
        patch.phone = body.phone === null ? null : String(body.phone);
    }
    if (body.preferences !== undefined && typeof body.preferences === "object") {
        patch.preferences = { ...user.preferences, ...body.preferences };
    }
    const currentPassword = body.current_password !== undefined ? String(body.current_password) : undefined;
    ...
    if (body.new_password !== undefined) {
        ...
        const cp = body.current_password !== undefined ? String(body.current_password) : undefined;
        if (!cp) {
    ...
    const updated = Object.keys(patch).length > 0 ? await repo.updateUserProfile(pool, userId, tenantId, patch) : user;
    if (!updated)
        throw new HttpError(404, "NOT_FOUND", "User not found");
    await cache.invalidateUserPermissions(tenantId, userId);
...
export async function inviteUser(tenantId, invitedBy, body) {
    ...
    const personnelId = body.personnel_id !== undefined && body.personnel_id !== null
