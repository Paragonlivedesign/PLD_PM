...
async function requirePersonnelId(tenantId, userId) {
    const user = await findUserById(pool, tenantId, userId);
    return user?.personnel_id ?? null;
}
timeRouter.post("/clock-in", requirePermission("time:clock:self"), asyncHandler(async (req, res) => {
    const pid = await requirePersonnelId(req.ctx.tenantId, req.ctx.userId);
    if (!pid) {
        res.status(400).json(singleError("validation", "User is not linked to personnel", 400).body);
        return;
    }
    try {
        ...
        if (open) {
            res.status(409).json(singleError("conflict", "Already clocked in; clock out first", 409).body);
            return;
        }
...
timeRouter.post("/clock-out", requirePermission("time:clock:self"), asyncHandler(async (req, res) => {
    const pid = await requirePersonnelId(req.ctx.tenantId, req.ctx.userId);
    if (!pid) {
        res.status(400).json(singleError("validation", "User is not linked to personnel", 400).body);
        return;
...
timeRouter.get("/entries", requireAnyPermission("time:read", "time:clock:self"), asyncHandler(async (req, res) => {
    const ctx = tryGetContext();
    const perm = ctx?.permissions ?? new Set();
    ...
    if (!canReadAll) {
        ...
        if (qPid && qPid !== selfPid) {
            res.status(403).json(singleError("forbidden", "May only list own time entries", 403).body);
            return;
        }
        filterPid = selfPid;
    }
...
payPeriodsRouter.get("/", requireAnyPermission("payroll:view_all", "payroll:view_own", "payroll:run", "payroll:export"), asyncHandler(async (req, res) => {
...
payPeriodsRouter.post("/", requirePermission("payroll:run"), asyncHandler(async (req, res) => {
    try {
...
payPeriodsRouter.get("/:id/statements", requireAnyPermission("payroll:view_all", "payroll:run"), asyncHandler(async (req, res) => {
    const periodId = routeParam(req.params.id);
    const n = await repo.countPayStatementsForPeriod(pool, req.ctx.tenantId, periodId);
    res.json(ok([], {
...
payrollRouter.get("/export", requirePermission("payroll:export"), asyncHandler(async (_req, res) => {
    res.json(ok({
        format: "adp_csv_v0",
