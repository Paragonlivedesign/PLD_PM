...
import { ENTITY_TYPES, FIELD_TYPES } from "./constants.js";
import * as service from "./service.js";
function parseEntityType(q) {
    if (typeof q !== "string")
        return null;
    return ENTITY_TYPES.includes(q) ? q : null;
}
function parseFieldType(v) {
    if (typeof v !== "string")
        return null;
    return FIELD_TYPES.includes(v) ? v : null;
}
export function customFieldsRouter(pool) {
    ...
    r.post("/", requirePermission("custom_fields.definitions.manage"), asyncHandler(async (req, res) => {
        ...
        const out = await service.createDefinition(pool, ctx.tenantId, ctx.userId, {
            ...
            validation_rules: b.validation_rules ?? null,
    ...
    r.put("/reorder", requirePermission("custom_fields.definitions.manage"), asyncHandler(async (req, res) => {
        const ctx = getContext();
        const b = req.body;
        const entityType = parseEntityType(b.entity_type);
        const orderedIds = Array.isArray(b.ordered_ids)
            ? b.ordered_ids.filter((x) => typeof x === "string")
            : null;
    ...
    r.put("/:id", requirePermission("custom_fields.definitions.manage"), asyncHandler(async (req, res) => {
        const ctx = getContext();
        ...
        const out = await service.updateDefinition(pool, ctx.tenantId, ctx.userId, defId, {
            ...
            ...(b.description !== undefined
                ...
                : {}),
            ...(b.validation_rules !== undefined ? { validation_rules: b.validation_rules } : {}),
            ...(b.default_value !== undefined ? { default_value: b.default_value } : {}),
            ...(Array.isArray(b.options) ? { options: b.options } : {}),
    ...
    r.delete("/:id", requirePermission("custom_fields.definitions.manage"), asyncHandler(async (req, res) => {
        const ctx = getContext();
        const defId = typeof req.params.id === "string" ? req.params.id : req.params.id[0];
