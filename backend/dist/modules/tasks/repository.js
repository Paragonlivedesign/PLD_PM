...
export async function getTaskById(client, tenantId, id) {
    const r = await client.query(`SELECT * FROM tasks WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`, [tenantId, id]);
    return r.rows[0] ? mapTaskRow(r.rows[0]) : null;
}
export async function listTasks(client, p) {
    const cond = ["t.tenant_id = $1", "t.deleted_at IS NULL"];
    const args = [p.tenantId];
    let n = 2;
    if (p.status) {
    ...
    if (p.search?.trim()) {
        ...
        cond.push(`(t.title ILIKE $${n} ESCAPE '\\' OR COALESCE(t.description,'') ILIKE $${n} ESCAPE '\\')`);
        args.push(term);
        n += 1;
    }
    const whereSql = cond.join(" AND ");
    const countR = await client.query(`SELECT count(*)::text AS c FROM tasks t WHERE ${whereSql}`, args);
...
export async function insertTask(client, p) {
    const r = await client.query(`INSERT INTO tasks (
...
export async function updateTaskRow(client, tenantId, id, patch) {
    const cur = await client.query(`SELECT * FROM tasks WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`, [tenantId, id]);
    if (!cur.rows[0])
        return null;
    ...
    const assigneePersonnelId = patch.assigneePersonnelId !== undefined ? patch.assigneePersonnelId : row.assignee_personnel_id;
    ...
    const completionPercent = patch.completionPercent !== undefined ? patch.completionPercent : row.completion_percent;
    ...
    const metadata = patch.metadata !== undefined
        ...
        : (row.metadata && typeof row.metadata === "object" && !Array.isArray(row.metadata)
            ? row.metadata
            : {});
    const r = await client.query(`UPDATE tasks SET
      title = $3,
      description = $4,
      status = $5,
      priority = $6,
...
export async function softDeleteTask(client, tenantId, id) {
    const r = await client.query(`UPDATE tasks SET deleted_at = NOW(), updated_at = NOW()
     WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`, [tenantId, id]);
    return (r.rowCount ?? 0) > 0;
}
export async function bulkUpdateTasks(client, tenantId, taskIds, patch) {
    let n = 0;
