...
eventFinancialRouter.get("/:id/budget", asyncHandler(async (req, res) => {
...
eventFinancialRouter.get("/:id/financials", asyncHandler(async (req, res) => {
    const ctx = getContext();
    try {
        listFinancialRecordsQuerySchema.parse(req.query);
        const result = await svc.listEventFinancialsApi(ctx.tenantId, String(req.params.id), req.query);
        res.status(200).json(ok(result.data, result.meta));
    }
    catch (e) {
