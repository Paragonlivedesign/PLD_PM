...
import { costReportQuerySchema, createFinancialRecordSchema, createInvoiceSchema, listFinancialRecordsQuerySchema, listInvoicesQuerySchema, updateFinancialRecordSchema, updateInvoiceSchema, } from "./schemas.js";
function parseCsv(s) {
    if (!s?.trim())
        return undefined;
    return s.split(",").map((x) => x.trim()).filter(Boolean);
}
function buildCategoryBuckets(aggs, kind) {
    const byCat = new Map();
...
function summaryFromAggs(aggs) {
    ...
    for (const row of aggs) {
        ...
        if (row.type === "cost") {
            if (row.status === "estimated")
                total_estimated_costs += amt;
            if (row.status === "actual")
                total_actual_costs += amt;
            if (row.status === "approved")
                total_approved_costs += amt;
        }
        else {
            if (row.status === "estimated")
                total_estimated_revenue += amt;
            if (row.status === "actual")
                total_actual_revenue += amt;
            if (row.status === "approved")
                total_approved_revenue += amt;
        }
    }
    return {
...
export async function getEventBudgetApi(tenantId, eventId) {
    ...
    const profit_margin_pct = total_revenue > 0 ? (net_profit / total_revenue) * 100 : 0;
...
function emitBudgetUpdated(tenantId, eventId, prev, next, trigger) {
...
export async function createFinancialRecordApi(tenantId, userId, body) {
    const parsed = createFinancialRecordSchema.parse(body);
    const ev = await getEventById(pool, tenantId, parsed.event_id);
    if (!ev)
        throw new HttpError(404, "NOT_FOUND", "Event not found", "event_id");
...
export async function listFinancialRecordsApi(tenantId, query) {
...
export async function updateFinancialRecordApi(tenantId, id, body) {
    ...
    if (parsed.amount != null)
        amountStr = String(parsed.amount);
    const mergedMeta = parsed.metadata != null
    ...
    const patch = {
        ...
        quantity: parsed.quantity !== undefined
            ? parsed.quantity != null
                ...
                : null
            : undefined,
        unitPrice: parsed.unit_price !== undefined
...
export async function deleteFinancialRecordApi(tenantId, id) {
    const existing = await repo.getFinancialRecordById(pool, tenantId, id);
    if (!existing)
        throw new HttpError(404, "NOT_FOUND", "Financial record not found");
    if (existing.source === "calculated") {
        throw new HttpError(409, "CONFLICT", "Calculated records cannot be deleted directly");
    }
    const locked = await repo.isFinancialRecordLockedByInvoice(pool, tenantId, id);
    if (locked) {
        throw new HttpError(409, "CONFLICT", "Record is attached to a non-draft invoice");
    }
    const eventId = existing.event_id;
    const prev = await repo.selectNetTotals(pool, tenantId, eventId);
    const prevNet = {
...
export async function listEventFinancialsApi(tenantId, eventId, query) {
    const ev = await getEventById(pool, tenantId, eventId);
    if (!ev)
        throw new HttpError(404, "NOT_FOUND", "Event not found");
    const q = listFinancialRecordsQuerySchema.parse(query);
...
function computeInvoiceTotals(lines, taxRate, discount, discountType) {
...
export async function createInvoiceApi(tenantId, userId, body) {
    ...
    const totals = computeInvoiceTotals(lineNums, parsed.tax_rate, parsed.discount ?? 0, parsed.discount_type ?? "fixed");
    const invoiceNumber = parsed.invoice_number ?? `INV-${randomUUID().replace(/-/g, "").slice(0, 12).toUpperCase()}`;
    const id = randomUUID();
...
export async function listInvoicesApi(tenantId, query) {
    ...
    const data = await Promise.all(slice.map(async (inv) => {
...
export async function getInvoiceApi(tenantId, id) {
    const inv = await repo.getInvoiceHeader(pool, tenantId, id);
    if (!inv)
...
export async function updateInvoiceApi(tenantId, userId, id, body) {
    ...
    if (inv.status !== "draft" &&
        ...
        throw new HttpError(409, "CONFLICT", "Only draft invoices can be modified for line items and pricing fields");
    }
    ...
    try {
        ...
        if (parsed.line_items && inv.status === "draft") {
            ...
            const tr = parsed.tax_rate !== undefined && parsed.tax_rate !== null
                ? parsed.tax_rate
                : inv.tax_rate != null
                    ? Number(inv.tax_rate)
                    : undefined;
            const disc = parsed.discount ?? Number(inv.discount);
            const dt = (parsed.discount_type ?? inv.discount_type);
        ...
        if (parsed.payment_terms !== undefined)
            patch.paymentTerms = parsed.payment_terms;
        if (parsed.paid_date !== undefined)
            patch.paidDate = parsed.paid_date;
...
export async function costReportApi(tenantId, query) {
...
export async function costReportCsvApi(tenantId, query) {
...
export async function recalculateEventCostsApi(tenantId, eventId, options) {
    const ev = await getEventById(pool, tenantId, eventId);
    if (!ev)
        throw new HttpError(404, "NOT_FOUND", "Event not found");
    ...
    try {
        await client.query("BEGIN");
        recordsDeleted = await repo.softDeleteUnlockedCalculatedCostRecordsForEvent(client, tenantId, eventId);
    ...
    if (prevNet.costs !== nextNet.costs ||
...
export async function getEventBudgetInternal(tenantId, eventId) {
    return getEventBudgetApi(tenantId, eventId);
}
export async function getEventCostsInternal(tenantId, eventId, _options) {
    const ev = await getEventById(pool, tenantId, eventId);
    if (!ev)
        return null;
    const { rows } = await repo.listFinancialRecords(pool, {
