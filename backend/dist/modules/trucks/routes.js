...
import { computeTruckRouteApi, createTruck, createTruckRouteApi, createAssignmentViaTruck, getTruck, getTruckAvailability, getTruckRoutesForEvent, listAssignmentsForTruck, listTruckRoutesApi, listTrucksApi, mintTruckRouteShareApi, refreshTruckRouteEtaApi, retireTruckApi, updateTruck, updateTruckRouteApi, } from "./trucks.service.js";
export const trucksRouter = Router();
trucksRouter.use(requestContextMiddleware);
trucksRouter.get("/", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const q = req.query;
    const type = typeof q.type === "string"
        ? q.type.split(",").map((s) => s.trim()).filter(Boolean)
        : undefined;
    const status = typeof q.status === "string"
        ? q.status.split(",").map((s) => s.trim()).filter(Boolean)
        : undefined;
    const sortByRaw = q.sort_by || "name";
    const sort_by = sortByRaw === "name" ||
        sortByRaw === "type" ||
        sortByRaw === "status" ||
    ...
    const result = await listTrucksApi({
        ...
        min_capacity_cubic_ft: q.min_capacity_cubic_ft != null ? Number(q.min_capacity_cubic_ft) : undefined,
        min_capacity_lbs: q.min_capacity_lbs != null ? Number(q.min_capacity_lbs) : undefined,
...
trucksRouter.post("/", requirePermission("trucks:create"), asyncHandler(async (req, res) => {
    const ctx = getContext();
...
trucksRouter.get("/:id/availability", asyncHandler(async (req, res) => {
    const ctx = getContext();
...
trucksRouter.get("/:id/assignments", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const q = req.query;
    const status = typeof q.status === "string"
        ? q.status.split(",").map((s) => s.trim()).filter(Boolean)
    ...
    const result = await listAssignmentsForTruck({
        tenantId: ctx.tenantId,
        truckId: id,
        date_range_start: typeof q.date_range_start === "string" ? q.date_range_start : undefined,
        date_range_end: typeof q.date_range_end === "string" ? q.date_range_end : undefined,
        status,
        limit,
        cursor: typeof q.cursor === "string" ? q.cursor : null,
    });
...
trucksRouter.post("/:id/assignments", requirePermission("trucks:assign"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
trucksRouter.get("/:id", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
trucksRouter.put("/:id", requirePermission("trucks:update"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
trucksRouter.delete("/:id", requirePermission("trucks:delete"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
truckRoutesRouter.get("/", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const q = req.query;
    const limit = Math.min(100, Math.max(1, Number(q.limit ?? 25) || 25));
...
truckRoutesRouter.post("/:id/compute-route", requirePermission("trucks:route"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
truckRoutesRouter.post("/:id/share", requirePermission("trucks:route"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
truckRoutesRouter.post("/:id/refresh-eta", requirePermission("trucks:route"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
truckRoutesRouter.post("/", requirePermission("trucks:route"), asyncHandler(async (req, res) => {
    const ctx = getContext();
...
truckRoutesRouter.put("/:id", requirePermission("trucks:route"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
truckRoutesRouter.get("/:eventId", asyncHandler(async (req, res) => {
    const ctx = getContext();
