...
import { countTrucks, decodeListCursor, encodeListCursor, findDuplicateNameOrPlate, getTruckRowById, insertTruck, listTrucks, mapTruckRow, retireTruck, updateTruckPartial, } from "./trucks.repository.js";
import { getAssignmentsForTruckInRange, getTruckAssignmentsByEventId, mapTruckAssignmentRow, } from "../scheduling/truck-assignments.repository.js";
import { createTruckAssignmentApi, listTruckAssignmentsApi, } from "../scheduling/truck-assignments.service.js";
import { countTruckRoutes, getTruckRouteById, getTruckRouteByShareToken, insertTruckRoute, listRoutesByEvent, listTruckRoutes, mapTruckRouteRow, setDriverShareToken, updateTruckRoutePartial, } from "./truck-routes.repository.js";
...
function isTruckStatus(s) {
    return TRUCK_STATUSES.includes(s);
}
const ROUTE_STATUSES = ["planned", "in_transit", "completed", "cancelled"];
function isRouteStatus(s) {
    return ROUTE_STATUSES.includes(s);
}
...
async function scheduleConflictHint(tenantId, eventId, estimatedArrivalIso) {
...
async function normalizeWaypointBatch(tenantId, raw) {
    ...
    for (const item of raw) {
        ...
        out.push({
            ...w,
            location,
            location_ref: lr ? refToJson(lr) : w.location_ref ?? {},
        });
    }
    return out;
}
...
export async function createTruck(input) {
    ...
    const licensePlate = input.body.license_plate != null ? String(input.body.license_plate).trim() || null : null;
    ...
    const data = await insertTruck(pool, {
        ...
        capacityCubicFt: input.body.capacity_cubic_ft != null ? Number(input.body.capacity_cubic_ft) : null,
        ...
        currentMileage: input.body.current_mileage != null ? Number(input.body.current_mileage) : null,
        insuranceExpiry: input.body.insurance_expiry != null ? String(input.body.insurance_expiry).slice(0, 10) : null,
        inspectionExpiry: input.body.inspection_expiry != null
            ? String(input.body.inspection_expiry).slice(0, 10)
            : null,
        notes: input.body.notes != null ? String(input.body.notes) : null,
        metadata: input.body.metadata && typeof input.body.metadata === "object"
            ? input.body.metadata
            : {},
    });
...
export async function getTruck(tenantId, id) {
...
export async function updateTruck(input) {
    ...
    const name = input.body.name != null ? String(input.body.name).trim() : undefined;
    ...
    const licensePlate = input.body.license_plate !== undefined
        ? input.body.license_plate === null
            ? null
            : String(input.body.license_plate).trim() || null
        : undefined;
    const dup = await findDuplicateNameOrPlate(pool, input.tenantId, name ?? cur.name, licensePlate !== undefined ? licensePlate : cur.license_plate, input.id);
    if (dup === "name") {
        return {
            ok: false,
            status: 409,
    ...
    if (input.body.vin !== undefined)
        patch.vin = input.body.vin === null ? null : String(input.body.vin);
    ...
    if (input.body.metadata !== undefined && typeof input.body.metadata === "object") {
        const existing = cur.metadata && typeof cur.metadata === "object"
            ? cur.metadata
            : {};
...
export async function createAssignmentViaTruck(input) {
    ...
    return createTruckAssignmentApi({
        ...
        notes: input.body.notes != null ? String(input.body.notes) : null,
        status: input.body.status != null ? String(input.body.status) : "tentative",
    });
}
...
export async function getTruckAvailability(tenantId, truckId, start, end) {
    const truck = await getTruckRowById(pool, tenantId, truckId);
    if (!truck)
        return null;
    const assignments = await getAssignmentsForTruckInRange(pool, tenantId, truckId, start, end);
    ...
    const outDays = days.map((date) => {
        ...
        const hit = assignments.find((a) => rangesOverlap(date, date, a.start_date.toString().slice(0, 10), a.end_date.toString().slice(0, 10)));
...
export async function getTruckRoutesForEvent(tenantId, eventId) {
    const ev = await getEventById(pool, tenantId, eventId);
    if (!ev)
        return null;
    ...
    for (const r of rows) {
        if (r.distance_miles)
            sumDist += Number(r.distance_miles);
        if (r.estimated_fuel_cost)
            sumCost += Number(r.estimated_fuel_cost);
    }
    return {
        data: {
            event_id: eventId,
...
export async function createTruckRouteApi(input) {
    ...
    const metaRaw = input.body.metadata && typeof input.body.metadata === "object"
        ? input.body.metadata
        : {};
    ...
    const row = await insertTruckRoute(pool, {
        ...
        eventId,
        truckId,
        assignmentId: input.body.assignment_id != null ? String(input.body.assignment_id) : null,
        driverId: input.body.driver_id != null ? String(input.body.driver_id) : null,
        origin,
        destination,
        ...
        distanceMiles: input.body.distance_miles != null ? Number(input.body.distance_miles) : null,
        estimatedFuelCost: input.body.estimated_fuel_cost != null
            ? Number(input.body.estimated_fuel_cost)
            : null,
        cargoDescription: input.body.cargo_description != null ? String(input.body.cargo_description) : null,
        notes: input.body.notes != null ? String(input.body.notes) : null,
        metadata: metaRaw,
        status: statusRaw,
    });
...
export async function updateTruckRouteApi(input) {
    ...
    if (input.body.origin !== undefined)
        patch.origin = String(input.body.origin);
    ...
    if (input.body.metadata !== undefined && typeof input.body.metadata === "object") {
        const cur = await pool
            .query(`SELECT metadata FROM truck_routes WHERE tenant_id = $1 AND id = $2`, [
            input.tenantId,
            input.id,
        ])
            .then((r) => r.rows[0]);
        const prev = cur?.metadata && typeof cur.metadata === "object"
            ? cur.metadata
            : {};
        patch.metadata = { ...prev, ...input.body.metadata };
    }
    if (input.body.status !== undefined)
        patch.status = String(input.body.status);
...
async function collectRoutePoints(tenantId, row) {
    const points = [];
    const pushCoord = (lat, lng) => {
        points.push({ lat, lng });
    };
    ...
    const oCoord = oRef
        ...
        : null;
    if (oCoord)
        pushCoord(oCoord.lat, oCoord.lng);
    else {
        const m = String(row.origin).match(/^(-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)$/);
        if (m)
            pushCoord(Number(m[1]), Number(m[2]));
    }
    ...
    const dCoord = dRef
        ...
        : null;
    if (dCoord)
        pushCoord(dCoord.lat, dCoord.lng);
    else {
...
function delayThresholdMinutes() {
...
export async function computeTruckRouteApi(input) {
    ...
    if (!withNames)
        return { ok: false, status: 404, errors: [{ code: "not_found", message: "Not found" }] };
    const prevEta = withNames.estimated_arrival instanceof Date
    ...
    const dep = withNames.departure_datetime instanceof Date
...
export async function getTruckRoutePublicByToken(token) {
...
export async function getTruckByIdInternal(truckId, tenantId, options) {
    const row = await getTruckRowById(pool, tenantId, truckId);
    if (!row)
        return null;
    if (row.status === "retired" && !options?.include_retired)
        return null;
    return mapTruckRow(row);
}
export async function getAvailableTrucksInternal(tenantId, startDate, endDate, options) {
    const p = {
        tenantId,
        type: options?.type,
    ...
    for (const row of rows) {
        if (row.status === "maintenance" || row.status === "retired")
            continue;
        const overlaps = await getAssignmentsForTruckInRange(pool, tenantId, row.id, startDate, endDate);
        const busy = overlaps.some((a) => a.status !== "cancelled");
        if (!busy)
            out.push(mapTruckRow(row));
    }
    return out;
}
export async function getTruckAssignmentsByEventInternal(eventId, tenantId, options) {
    const rows = await getTruckAssignmentsByEventId(pool, tenantId, eventId, options?.status, options?.include_cancelled === true);
