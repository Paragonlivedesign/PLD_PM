...
export function encodeTravelCursor(row, sortBy) {
    let v;
    switch (sortBy) {
        case "personnel_name":
...
export function decodeTravelCursor(raw) {
    if (!raw?.trim())
        return null;
    try {
...
export async function insertTravelRecord(client, row) {
    const r = await client.query(`${baseSelect()} WHERE FALSE`, []);
    void r;
    const ins = await client.query(`INSERT INTO travel_records (
...
export async function getTravelById(client, tenantId, id) {
    const r = await client.query(`${baseSelect()}
     WHERE tr.tenant_id = $1 AND tr.id = $2 AND tr.deleted_at IS NULL`, [tenantId, id]);
    const row = r.rows[0];
    return row ? mapRowToResponse(row) : null;
}
export async function updateTravelRecordRow(client, tenantId, id, patch) {
    const sets = [];
    const vals = [];
    let i = 1;
    const push = (frag, val) => {
    ...
    if (patch.travel_type != null)
        push("travel_type = $N", patch.travel_type);
    if (patch.direction != null)
        push("direction = $N", patch.direction);
    if (patch.departure_location != null)
        push("departure_location = $N", patch.departure_location);
    if (patch.arrival_location != null)
        push("arrival_location = $N", patch.arrival_location);
    if (patch.departure_datetime != null)
        push("departure_datetime = $N::timestamptz", patch.departure_datetime);
    if (patch.arrival_datetime != null)
        push("arrival_datetime = $N::timestamptz", patch.arrival_datetime);
    if (patch.carrier !== undefined)
        push("carrier = $N", patch.carrier);
    if (patch.booking_reference !== undefined)
        push("booking_reference = $N", patch.booking_reference);
    if (patch.seat_preference !== undefined)
        push("seat_preference = $N", patch.seat_preference);
    if (patch.cost !== undefined)
        push("cost = $N", patch.cost);
    if (patch.currency != null)
        push("currency = $N", patch.currency);
    if (patch.status != null)
        push("status = $N", patch.status);
    if (patch.notes !== undefined)
        push("notes = $N", patch.notes);
    ...
    const tid = i;
    const iid = i + 1;
    await client.query(`UPDATE travel_records SET ${sets.join(", ")}
     WHERE tenant_id = $${tid} AND id = $${iid}::uuid AND deleted_at IS NULL`, vals);
    return getTravelById(client, tenantId, id);
}
export async function softDeleteTravel(client, tenantId, id) {
    const r = await client.query(`UPDATE travel_records SET deleted_at = NOW(), updated_at = NOW()
...
export async function countTravelRecords(client, p) {
    const { where, vals } = buildWhere(p);
    const r = await client.query(`SELECT COUNT(*)::text AS n FROM travel_records tr
     INNER JOIN events e ON e.id = tr.event_id AND e.tenant_id = tr.tenant_id AND e.deleted_at IS NULL
     INNER JOIN personnel p ON p.id = tr.personnel_id AND p.tenant_id = tr.tenant_id AND p.deleted_at IS NULL
     WHERE ${where}`, vals);
...
function buildWhere(p) {
    const parts = ["tr.tenant_id = $1", "tr.deleted_at IS NULL"];
    ...
    if (p.search?.trim()) {
        ...
        parts.push(`(tr.departure_location ILIKE $${i} ESCAPE '\\' OR tr.arrival_location ILIKE $${i} ESCAPE '\\' OR tr.carrier ILIKE $${i} ESCAPE '\\' OR tr.booking_reference ILIKE $${i} ESCAPE '\\')`);
        vals.push(q);
        i++;
    }
...
function cursorClause(sortBy, sortOrder, cursorVal, cursorId) {
...
export async function listTravelRecords(client, p) {
    const { where, vals } = buildWhere(p);
    ...
    const r = await client.query(`${baseSelect()}
     ...
     ${ord}
     LIMIT $${allVals.length}`, allVals);
    return r.rows.map((row) => mapRowToResponse(row));
}
export async function listTravelForEvent(client, tenantId, eventId, options) {
...
export async function listTravelForPersonnel(client, tenantId, personnelId, options) {
