...
import { createCrewAssignmentApi, deleteCrewAssignmentApi, getCrewAssignment, listCrewAssignmentsApi, updateCrewAssignmentApi, } from "./crew-assignments.service.js";
import { createTruckAssignmentApi, deleteTruckAssignmentApi, getTruckAssignment, listTruckAssignmentsApi, updateTruckAssignmentApi, } from "./truck-assignments.service.js";
export const assignmentsRouter = Router();
assignmentsRouter.use(requestContextMiddleware);
assignmentsRouter.post("/crew", requirePermission("scheduling:create"), asyncHandler(async (req, res) => {
    ...
    const r = await createCrewAssignmentApi({
        ...
        dayRateOverride: b.day_rate_override !== undefined && b.day_rate_override !== null
            ? Number(b.day_rate_override)
            : null,
        perDiemOverride: b.per_diem_override !== undefined && b.per_diem_override !== null
...
assignmentsRouter.get("/crew", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const q = req.query;
    ...
    const sortBy = sortByRaw === "personnel_name" ||
...
assignmentsRouter.get("/crew/:id", asyncHandler(async (req, res) => {
    const ctx = getContext();
...
assignmentsRouter.put("/crew/:id", requirePermission("scheduling:update"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
    const b = req.body;
    const r = await updateCrewAssignmentApi(ctx.tenantId, ctx.userId, id, {
        ...
        departmentId: b.department_id !== undefined
            ...
            : undefined,
        departmentName: b.department_name !== undefined
        ...
        startTime: b.start_time !== undefined
            ? b.start_time === null
                ...
                : String(b.start_time)
            : undefined,
        endTime: b.end_time !== undefined ? (b.end_time === null ? null : String(b.end_time)) : undefined,
        dayRateOverride: b.day_rate_override !== undefined
            ? b.day_rate_override === null
                ? null
                : Number(b.day_rate_override)
            : undefined,
        perDiemOverride: b.per_diem_override !== undefined
...
assignmentsRouter.delete("/crew/:id", requirePermission("scheduling:delete"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
assignmentsRouter.post("/bulk", requirePermission("scheduling:create"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const b = req.body;
    const raw = Array.isArray(b.assignments) ? b.assignments : [];
    const strategyRaw = b.conflict_strategy ?? "fail";
    const conflictStrategy = strategyRaw === "warn" || strategyRaw === "skip" ? strategyRaw : "fail";
    const assignments = raw.map((item) => {
        const x = item;
        return {
            type: x.type === "truck" ? "truck" : "crew",
...
assignmentsRouter.post("/truck", requirePermission("scheduling:create"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const b = req.body;
...
assignmentsRouter.get("/truck", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const q = req.query;
    const statusCsv = typeof q.status === "string" ? q.status : undefined;
    const status = statusCsv?.split(",").map((s) => s.trim()).filter(Boolean);
...
assignmentsRouter.get("/truck/:id", asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
...
assignmentsRouter.put("/truck/:id", requirePermission("scheduling:update"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
    const b = req.body;
...
assignmentsRouter.delete("/truck/:id", requirePermission("scheduling:delete"), asyncHandler(async (req, res) => {
    const ctx = getContext();
    const id = String(req.params.id);
