...
export function mapCrewAssignmentRow(r, dayRate, perDiem) {
    const start_date = isoDate(r.start_date);
    const end_date = isoDate(r.end_date);
    const total_days = inclusiveDays(start_date, end_date);
    const dr = r.day_rate_override != null && r.day_rate_override !== ""
        ? Number(r.day_rate_override)
        : dayRate;
    const pd = r.per_diem_override != null && r.per_diem_override !== ""
        ? Number(r.per_diem_override)
        : perDiem;
    const day_rate_override = r.day_rate_override != null && r.day_rate_override !== ""
        ? dec(Number(r.day_rate_override))
        : null;
    const per_diem_override = r.per_diem_override != null && r.per_diem_override !== ""
...
export async function findOverlappingCrewAssignments(client, tenantId, personnelId, startDate, endDate, excludeId) {
    const r = await client.query(`SELECT * FROM crew_assignments
     WHERE tenant_id = $1 AND personnel_id = $2 AND deleted_at IS NULL
       AND status <> 'cancelled'
       AND start_date <= $4::date AND end_date >= $3::date
       AND ($5::uuid IS NULL OR id <> $5)`, [tenantId, personnelId, startDate, endDate, excludeId ?? null]);
    return r.rows;
}
export async function listAllActiveCrewAssignmentsForPersonnel(client, tenantId, personnelId) {
    const r = await client.query(`SELECT * FROM crew_assignments
     WHERE tenant_id = $1 AND personnel_id = $2 AND deleted_at IS NULL AND status <> 'cancelled'
     ORDER BY start_date ASC, end_date ASC, id ASC`, [tenantId, personnelId]);
    return r.rows;
}
export async function selectDistinctEventIdsForPersonnelCrew(client, tenantId, personnelId) {
    const r = await client.query(`SELECT DISTINCT event_id FROM crew_assignments
     WHERE tenant_id = $1 AND personnel_id = $2 AND deleted_at IS NULL AND status <> 'cancelled'`, [tenantId, personnelId]);
    return r.rows.map((x) => x.event_id);
}
export async function insertCrewAssignment(client, p) {
    await client.query(`INSERT INTO crew_assignments (
    ...
    if (!full)
        throw new Error("insert crew_assignment failed");
    return full;
}
export async function getCrewAssignmentById(client, tenantId, id) {
    const r = await client.query(`SELECT * FROM crew_assignments
     WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`, [tenantId, id]);
    return r.rows[0] ?? null;
}
export async function countCrewAssignments(client, p) {
    const { where, vals } = buildCaWhere(p);
    const r = await client.query(`SELECT count(*)::int AS c FROM crew_assignments ca ${where}`, vals);
    return r.rows[0]?.c ?? 0;
}
function buildCaWhere(p) {
...
function orderClause(sortBy, sortOrder) {
    ...
    const col = sortBy === "personnel_name"
        ? "ca.personnel_name"
        : sortBy === "event_name"
            ? "ca.event_name"
            : sortBy === "created_at"
...
export async function listCrewAssignments(client, p) {
    const { where, vals } = buildCaWhere(p);
    ...
    const r = await client.query(`SELECT ca.* FROM crew_assignments ca
     ${where}
     ORDER BY ${ord}
     OFFSET $${offIdx} LIMIT $${limIdx}`, v);
    return r.rows;
}
export async function updateCrewAssignment(client, tenantId, id, patch) {
    const cur = await client.query(`SELECT * FROM crew_assignments WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`, [tenantId, id]);
    const row = cur.rows[0];
    if (!row)
        return null;
    const role = patch.role ?? row.role;
    const departmentId = patch.departmentId !== undefined ? patch.departmentId : row.department_id;
    const departmentName = patch.departmentName !== undefined ? patch.departmentName : row.department_name;
    ...
    const dayRateOverride = patch.dayRateOverride !== undefined ? patch.dayRateOverride : row.day_rate_override != null
        ? Number(row.day_rate_override)
        : null;
    const perDiemOverride = patch.perDiemOverride !== undefined ? patch.perDiemOverride : row.per_diem_override != null
    ...
    await client.query(`UPDATE crew_assignments SET
      role = $3,
      department_id = $4,
      department_name = $5,
      start_date = $6::date,
...
export async function softDeleteCrewAssignment(client, tenantId, id) {
    const r = await client.query(`UPDATE crew_assignments SET deleted_at = NOW(), updated_at = NOW()
...
export async function listCrewAssignmentsByEventId(client, tenantId, eventId, statusFilter, includeCancelled = false) {
    ...
    const r = await client.query(`SELECT ca.* FROM crew_assignments ca
     ...
     ORDER BY ca.start_date ASC`, vals);
    return r.rows;
}
export async function listCrewAssignmentsByPersonnelInRange(client, tenantId, personnelId, startDate, endDate, statusFilter, includeCancelled = false) {
    const vals = [tenantId, personnelId, startDate, endDate];
    let extra = "";
    if (statusFilter?.length) {
...
export async function listCrewAssignmentsOverlappingRange(client, tenantId, rangeStart, rangeEnd, filters) {
    ...
    const r = await client.query(`SELECT ca.* FROM crew_assignments ca WHERE ${parts.join(" AND ")} ORDER BY ca.start_date ASC`, vals);
    return r.rows;
}
export async function countFutureCrewAssignmentsForPersonnel(client, tenantId, personnelId, fromDate) {
    const r = await client.query(`SELECT count(*)::int AS c FROM crew_assignments
     WHERE tenant_id = $1 AND personnel_id = $2 AND deleted_at IS NULL
       AND status IN ('tentative', 'confirmed')
       AND end_date >= $3::date`, [tenantId, personnelId, fromDate]);
    return r.rows[0]?.c ?? 0;
}
export async function expandCrewAssignmentsToDays(client, tenantId, personnelId, startDate, endDate) {
    const rows = await listCrewAssignmentsByPersonnelInRange(client, tenantId, personnelId, startDate, endDate);
    const out = [];
...
export async function listCrewAssignmentsByPersonnelIdsInRange(client, tenantId, personnelIds, startDate, endDate) {
    ...
    const r = await client.query(`SELECT ca.* FROM crew_assignments ca
     ...
     ORDER BY ca.personnel_id, ca.start_date ASC`, [tenantId, personnelIds, startDate, endDate]);
    return r.rows;
}
/** Expand many assignments into per-personnel_id day slices (within [startDate, endDate]). */
export function expandCrewAssignmentsToDaysByPersonnel(rows, startDate, endDate) {
    const map = new Map();
    for (const row of rows) {
