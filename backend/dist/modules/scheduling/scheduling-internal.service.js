...
import { listCrewAssignmentsByEventId, listCrewAssignmentsByPersonnelInRange, mapCrewAssignmentRow, } from "./crew-assignments.repository.js";
import { listConflicts, mapConflictRow, } from "./conflicts.repository.js";
import { getTruckAssignmentsByEventId, listTruckAssignmentsOverlappingRange, mapTruckAssignmentRow, } from "./truck-assignments.repository.js";
export async function getAssignmentsByEvent(eventId, tenantId, options) {
...
export async function getAssignmentsByPersonnel(personnelId, tenantId, options) {
    const start = options?.date_range_start ?? "1970-01-01";
    const end = options?.date_range_end ?? "2099-12-31";
    const rows = await listCrewAssignmentsByPersonnelInRange(pool, tenantId, personnelId, start, end, options?.status, options?.includeCancelled ?? false);
    const out = [];
    for (const r of rows) {
        const rates = await Promise.all([
            getDayRate(r.personnel_id, tenantId),
...
export async function getAssignmentsByTruck(truckId, tenantId, options) {
    const raw = await listTruckAssignmentsOverlappingRange(pool, tenantId, options?.date_range_start ?? "1970-01-01", options?.date_range_end ?? "2099-12-31", {
        truckId,
...
export async function getConflicts(tenantId, options) {
    const p = {
        tenantId,
...
export async function getAssignmentDays(eventId, tenantId, options) {
    const type = options?.type ?? "all";
    let crewDays = 0;
    let crewCost = 0;
    let crewPerDiem = 0;
