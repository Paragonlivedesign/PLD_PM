export function totalBlockers(b) {
    return (b.crew_assignments +
        b.truck_assignments +
        b.truck_routes +
        b.travel_records +
        b.financial_records +
 ...
 */
export async function summarizeEventDeleteBlockers(db, tenantId, eventId) {
    const { rows } = await db.query(`SELECT
       ...
       (SELECT COUNT(*)::int FROM tasks
         WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL) AS tasks_linked`, [tenantId, eventId]);
    const r = rows[0];
    return {
 ...
 */
export async function purgeEventRelatedData(db, tenantId, eventId) {
    ...
    try {
        ...
        await client.query(`UPDATE financial_records SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE crew_assignments SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE truck_assignments SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE truck_routes SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE travel_records SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE time_entries SET event_id = NULL, updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2`, [tenantId, eventId]);
        await client.query(`UPDATE tasks SET event_id = NULL, updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2`, [tenantId, eventId]);
        ...
        await client.query(`UPDATE documents SET deleted_at = NOW(), updated_at = NOW()
       WHERE tenant_id = $1 AND event_id = $2 AND deleted_at IS NULL`, [tenantId, eventId]);
        await client.query(`UPDATE scheduling_conflicts
       SET status = 'resolved', resolved_at = COALESCE(resolved_at, NOW())
       WHERE tenant_id = $1 AND status = 'active'
         AND (event_id_1 = $2 OR event_id_2 = $2)`, [tenantId, eventId]);
