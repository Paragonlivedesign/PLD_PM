...
function docCategoryFromTemplate(tplCat) {
    if (DOC_CATEGORIES.has(tplCat))
        return tplCat;
    return "other";
}
...
export function toDocumentApi(row, storage) {
...
export async function uploadDocumentApi(storage, input) {
    ...
    if (cat === "rider" && input.fields.rider_items_json?.trim() && row.event_id) {
        const riderEventId = row.event_id;
        try {
            const parsed = JSON.parse(input.fields.rider_items_json);
            if (Array.isArray(parsed) && parsed.length > 0) {
                const items = parsed.map((it) => ({
                    ...
                    category: it.category && RIDER_CATS.has(String(it.category)) ? String(it.category) : "other",
...
export async function listDocumentsApi(storage, query) {
    const ctx = getContext();
    const limit = Math.min(100, Math.max(1, Number(query.limit ?? 25) || 25));
    const sortByRaw = query.sort_by ?? "created_at";
    const sort_by = sortByRaw === "name" || sortByRaw === "category" || sortByRaw === "size"
        ? sortByRaw
        : "created_at";
    ...
    const cursor = decoded && decoded.sortBy === sort_by
...
export async function getDocumentApi(storage, id, req) {
    const ctx = getContext();
...
export async function deleteDocumentApi(storage, id) {
    const ctx = getContext();
    const row = await repo.getDocumentById(pool, ctx.tenantId, id);
    if (!row)
        throw new HttpError(404, "NOT_FOUND", "Document not found");
...
export async function streamDocumentFile(storage, token, expectedDocumentId) {
    const parsed = verifyDownloadToken(token);
    if (!parsed || parsed.documentId !== expectedDocumentId)
        return null;
...
export async function generateDocumentApi(storage, body, req) {
    const ctx = getContext();
    const t0 = Date.now();
    ...
    const category = body.category && DOC_CATEGORIES.has(body.category)
        ? body.category
...
export async function getDocumentsByEvent(db, tenantId, eventId, options) {
...
export async function getTemplates(db, tenantId, options) {
...
function toTemplateDetailApi(t) {
...
export async function generateDocumentInternal(db, tenantId, userId, templateId, context, storage) {
    ...
    const category = context.category && DOC_CATEGORIES.has(context.category)
        ? context.category
        : docCategoryFromTemplate(tpl.category);
    const vis = context.visibility && VISIBILITY.has(context.visibility) ? context.visibility : "internal";
    const id = randomUUID();
    const relKey = `documents/generated/${id}.html`;
    const { Readable } = await import("node:stream");
    const stream = Readable.from([html], { objectMode: false });
...
export function listTemplateVariableCatalogApi() {
    return [...TEMPLATE_VARIABLE_CATALOG];
}
export async function listTemplatesApi(query) {
    const ctx = getContext();
    const sortByRaw = query.sort_by ?? "name";
    const sort_by = sortByRaw === "created_at" || sortByRaw === "category" ? sortByRaw : "name";
...
export async function createTemplateApi(body) {
    ...
    if (!name || name.length > 255)
        throw new HttpError(400, "VALIDATION", "Invalid name", "name");
...
export async function updateTemplateApi(id, body) {
    const ctx = getContext();
    ...
    if (body.format !== undefined)
        patch.format = String(body.format);
...
function decodeRiderCursor(raw) {
    if (!raw?.trim())
        return 0;
    try {
...
function encodeRiderCursor(offset) {
    return Buffer.from(String(offset), "utf8").toString("base64url");
}
export async function listRiderItemsApi(query) {
    const ctx = getContext();
...
export async function updateRiderItemApi(id, body) {
    const ctx = getContext();
    const existing = await repo.getRiderById(pool, ctx.tenantId, id);
    if (!existing)
        throw new HttpError(404, "NOT_FOUND", "Rider item not found");
    const patch = {};
    if (body.category !== undefined) {
    ...
    if (body.quantity !== undefined) {
        ...
        if (!Number.isInteger(q) || q < 1)
            throw new HttpError(400, "VALIDATION", "Invalid quantity", "quantity");
        patch.quantity = q;
    }
...
export async function batchCreateRiderItemsApi(body) {
    ...
    if (!body.items?.length)
        throw new HttpError(400, "VALIDATION", "items required", "items");
    ...
    const rows = body.items.map((it) => ({
        ...
        event_id: body.event_id,
        description: String(it.description ?? "").slice(0, 2000) || "Item",
        category: it.category && RIDER_CATS.has(it.category) ? it.category : "other",
        quantity: typeof it.quantity === "number" && it.quantity > 0 ? Math.floor(it.quantity) : 1,
        status: "pending",
        notes: null,
        assigned_to: null,
...
// --- email drafts ---
export async function createEmailDraftApi(body) {
    const ctx = getContext();
    ...
    if (Array.isArray(body.attachments)) {
        for (const aid of body.attachments) {
