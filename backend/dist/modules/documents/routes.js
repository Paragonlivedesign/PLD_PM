...
documentsRouter.post("/upload", requirePermission("documents:upload"), upload.single("file"), resumeRequestContextMiddleware, asyncHandler(async (req, res) => {
    ...
    if (!category)
        throw new HttpError(400, "VALIDATION", "category is required", "category");
...
documentsRouter.post("/generate", requirePermission("documents:generate"), asyncHandler(async (req, res) => {
    const body = req.body;
    const template_id = String(body.template_id ?? "");
    const event_id = String(body.event_id ?? "");
    assertUuid(template_id, "template_id");
    assertUuid(event_id, "event_id");
    const result = await svc.generateDocumentApi(storage, {
        template_id,
        event_id,
        output_format: body.output_format != null ? String(body.output_format) : undefined,
        data_overrides: body.data_overrides && typeof body.data_overrides === "object"
            ? body.data_overrides
            : undefined,
...
documentsRouter.get("/:id/file", asyncHandler(async (req, res) => {
    ...
    if (!file)
        throw new HttpError(403, "FORBIDDEN", "Invalid or expired download link");
...
documentsRouter.delete("/:id", requirePermission("documents:delete"), asyncHandler(async (req, res) => {
    const id = paramId(req.params.id);
    assertUuid(id);
...
templatesRouter.get("/variable-catalog", asyncHandler(async (_req, res) => {
...
templatesRouter.post("/", requirePermission("templates:create"), asyncHandler(async (req, res) => {
    const data = await svc.createTemplateApi(req.body);
    res.status(201).json(ok(data));
}));
templatesRouter.put("/:id", requirePermission("templates:update"), asyncHandler(async (req, res) => {
    const id = paramId(req.params.id);
    assertUuid(id);
    const { data, meta } = await svc.updateTemplateApi(id, req.body);
    res.status(200).json(ok(data, meta));
}));
export const riderItemsRouter = Router();
riderItemsRouter.use(requestContextMiddleware);
riderItemsRouter.get("/", asyncHandler(async (req, res) => {
    const q = req.query;
    const result = await svc.listRiderItemsApi(q);
    res.status(200).json(ok(result.data, result.meta));
}));
riderItemsRouter.post("/", requirePermission("documents:update"), asyncHandler(async (req, res) => {
    const body = req.body;
...
riderItemsRouter.put("/:id", requirePermission("documents:update"), asyncHandler(async (req, res) => {
    const id = paramId(req.params.id);
    assertUuid(id);
    const data = await svc.updateRiderItemApi(id, req.body);
    res.status(200).json(ok(data));
}));
export const emailDraftsRouter = Router();
emailDraftsRouter.use(requestContextMiddleware);
emailDraftsRouter.post("/", requirePermission("documents:generate"), asyncHandler(async (req, res) => {
    const data = await svc.createEmailDraftApi(req.body);
    res.status(201).json(ok(data));
}));
