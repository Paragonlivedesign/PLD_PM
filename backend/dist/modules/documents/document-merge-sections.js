 ...
 */
export async function buildHtmlMergeSectionsForEvent(tenantId, eventId) {
    ...
    const crewRows = asg.crew
        .slice(0, 50)
        .map((a) => `<tr><td>${esc(a.personnel_name ?? "—")}</td><td>${esc(String(a.role ?? "—"))}</td><td>${esc(String(a.start_date ?? "—"))}</td><td>${esc(String(a.end_date ?? "—"))}</td><td>${esc(String(a.status ?? "—"))}</td></tr>`)
        .join("");
    const truckRows = asg.truck
        .slice(0, 30)
        .map((t) => `<tr><td>${esc(t.truck_name || "—")}</td><td>${esc(String(t.start_date ?? "—"))}</td><td>${esc(String(t.end_date ?? "—"))}</td><td>${esc(String(t.status ?? "—"))}</td></tr>`)
        .join("");
    const schedule_section = !crewRows && !truckRows
    ...
    const travel_section = travel_slice.length === 0
        ...
        : `<table border="1" cellpadding="6"><thead><tr><th>Personnel</th><th>Direction</th><th>Departure</th><th>Status</th></tr></thead><tbody>${travel_slice
            .map((r) => `<tr><td>${esc(String(r.personnel_name ?? "—"))}</td><td>${esc(String(r.direction ?? "—"))}</td><td>${esc(String(r.departure_datetime ?? "—"))}</td><td>${esc(String(r.status ?? "—"))}</td></tr>`)
            .join("")}</tbody></table>`;
    let financial_section = "<p><em>Budget data unavailable.</em></p>";
    if (budgetWrap?.data) {
        const d = budgetWrap.data;
