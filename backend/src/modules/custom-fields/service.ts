...
import { FIELD_KEY_REGEX, MAX_CUSTOM_FIELDS_JSON_BYTES, type EntityType, type FieldType } from "./constants.js";
import * as repo from "./repository.js";
import { getTierLimits } from "./tier.js";
import {
  validateDefaultValueForCreate,
...
function mergeSelectOptions(
  existing: SelectOption[] | null,
  incoming: SelectOption[] | null | undefined,
): SelectOption[] | null {
...
export async function getFieldDefinitions(
  ...
  options?: { include_deprecated?: boolean },
...
export async function getFieldsForSearch(
  pool: Pool,
  tenantId: string,
  entityType: EntityType,
): Promise<
  { field_key: string; label: string; field_type: string; options: SelectOption[] | null }[]
> {
...
export async function validateCustomFields(
...
): Promise<ValidateCustomFieldsResult> {
  ...
  for (const def of defs) {
    let v: unknown = values[def.field_key];
    ...
    if (r.ok) cleaned[def.field_key] = r.value;
    else
      errors.push({
        field_key: def.field_key,
        code: r.code,
...
export async function createDefinition(
  ...
  | { ok: true; data: CustomFieldDefinitionResponse }
...
> {
  ...
  if (body.default_value !== undefined && body.default_value !== null) {
    const dv = validateDefaultValueForCreate(
      body.field_type,
      body.default_value,
      body.validation_rules ?? null,
      body.options ?? null,
    );
    if (!dv.ok) return { ok: false, status: 400, code: "invalid_default", message: dv.message };
  }
  ...
  const displayOrder =
    body.display_order != null ? body.display_order : await repo.nextDisplayOrder(pool, tenantId, body.entity_type);
...
export async function updateDefinition(
  ...
  | {
      ...
      meta: { previous_version: number; new_version: number };
    }
  | { ok: false; status: number; code: string; message: string }
> {
  ...
  const mergedOptions =
    patch.options !== undefined ? mergeSelectOptions(prev.options, patch.options) : undefined;
  if (mergedOptions && mergedOptions.length > limits.maxSelectOptionsPerField) {
    return { ok: false, status: 400, code: "too_many_options", message: "Too many select options for tier" };
  }
  const nextDef: CustomFieldDefinitionResponse = {
    ...prev,
  ...
  if (patch.default_value !== undefined && patch.default_value !== null) {
    const dv = validateDefaultValueForCreate(
      prev.field_type,
      patch.default_value,
      nextDef.validation_rules,
      nextDef.options,
    );
    if (!dv.ok) return { ok: false, status: 400, code: "invalid_default", message: dv.message };
  }
  const updated = await repo.updateDefinitionRow(
    pool,
    tenantId,
    id,
    {
  ...
  if (!updated) return { ok: false, status: 404, code: "not_found", message: "Definition not found" };
  ...
  publishFieldDefinitionUpdated({
    ...
    previous_values: Object.fromEntries(changed.map((k) => [k, (prev as Record<string, unknown>)[k]])),
    new_values: Object.fromEntries(changed.map((k) => [k, (updated as Record<string, unknown>)[k]])),
...
export async function deleteDefinition(
  ...
  | { ok: true; data: { id: string; deleted_at: string } }
  | { ok: false; status: number; code: string; message: string }
> {
  const prev = await repo.getDefinitionById(pool, tenantId, id);
  if (!prev || prev.deleted_at) {
  ...
  if (!del) return { ok: false, status: 404, code: "not_found", message: "Definition not found" };
  publishFieldDefinitionDeleted({
