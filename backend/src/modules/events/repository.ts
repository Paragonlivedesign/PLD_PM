...
type DbEventRow = {
  ...
  load_in_date: Date | string | null;
  load_out_date: Date | string | null;
  status: string;
  phase: string;
...
export function mapEventRow(r: DbEventRow): EventResponse {
  ...
  return {
    ...
    deleted_at: r.deleted_at
      ? isoDateTime(
          r.deleted_at instanceof Date ? r.deleted_at : new Date(r.deleted_at),
        )
      : null,
  };
}
...
export async function findDuplicateEvent(
  ...
  const r = await client.query(
    ...
    [tenantId, clientId, name, startDate, endDate, excludeId ?? null],
  );
...
export async function insertEvent(
  ...
  return mapEventRow(r.rows[0] as DbEventRow);
}
...
export async function getEventById(
  ...
  const r = await client.query<DbEventRow>(
    `SELECT * FROM events WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`,
    [tenantId, id],
  );
...
export type ListEventsParams = {
  ...
  dateRangeEnd?: string;
  search?: string;
  sortBy: "name" | "start_date" | "end_date" | "created_at" | "updated_at";
  sortOrder: "asc" | "desc";
  limit: number;
...
export async function countEvents(
  client: Pool | PoolClient,
  p: Omit<ListEventsParams, "sortBy" | "sortOrder" | "limit" | "cursorVal" | "cursorId">,
): Promise<number> {
...
function buildWhere(
  ...
  const values: unknown[] = [p.tenantId];
  let i = 2;
  ...
  if (p.search?.trim()) {
    parts.push(
      `(e.name ILIKE $${i} OR e.description ILIKE $${i} OR e.tags::text ILIKE $${i})`,
    );
...
export async function listEvents(
  ...
  `;
  v.push(p.limit);
  const r = await client.query<DbEventRow>(sql, v);
...
export async function updateEventRow(
  ...
  if (patch.venue_id !== undefined) add("venue_id", patch.venue_id);
  if (patch.primary_contact_id !== undefined) add("primary_contact_id", patch.primary_contact_id);
  if (patch.start_date !== undefined) add("start_date", patch.start_date);
  if (patch.end_date !== undefined) add("end_date", patch.end_date);
  if (patch.load_in_date !== undefined) add("load_in_date", patch.load_in_date);
  if (patch.load_out_date !== undefined) add("load_out_date", patch.load_out_date);
  ...
  if (sets.length === 0) {
    if (expectedUpdatedAt) {
      const chk = await client.query<DbEventRow>(
        `SELECT * FROM events WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL
  ...
  const r = await client.query<DbEventRow>(sql, vals);
  return r.rows[0] ? mapEventRow(r.rows[0]) : null;
}

export async function updateEventPhaseRow(
  client: Pool | PoolClient,
  tenantId: string,
  id: string,
  phase: EventPhase,
  ...
  const r = await client.query<DbEventRow>(
    `UPDATE events SET phase = $3, updated_at = NOW()
...
export async function softDeleteEvent(
  ...
  return {
    ...
    deleted_at: isoDateTime(
      r.rows[0].deleted_at instanceof Date
        ? r.rows[0].deleted_at
...
export async function selectEventsByDateRange(
  client: Pool | PoolClient,
  tenantId: string,
  start: string,
  end: string,
  ...
  const vals: unknown[] = [tenantId, start, end];
  ...
  const r = await client.query<DbEventRow>(
    `SELECT e.* FROM events e ${w} ORDER BY e.start_date ASC`,
    vals,
  );
  return r.rows.map(mapEventRow);
}

export async function selectEventsByClient(
  client: Pool | PoolClient,
  tenantId: string,
  clientId: string,
  status?: string[],
  ...
  const r = await client.query<DbEventRow>(
    `SELECT e.* FROM events e ${where} ORDER BY e.start_date ASC LIMIT ${limit}`,
    values,
  );
  return r.rows.map(mapEventRow);
}
