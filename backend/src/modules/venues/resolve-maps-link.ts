...
export type ResolveMapsLinkResult = {
  ...
  partial: boolean;
  source: "google" | "regex" | "none";
};

function timezoneFromCoords(lat: number, lng: number): string | null {
  try {
...
export function parseCoordsFromMapsUrl(urlRaw: string): { lat: number; lng: number } | null {
  const u = urlRaw.trim();
  if (!u || u.length > 8000) return null;
...
export async function resolveMapsLink(urlRaw: string): Promise<ResolveMapsLinkResult> {
