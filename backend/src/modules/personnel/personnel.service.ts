...
import {
  ...
  getAssignmentsForPersonnelIdsDateRange,
  hasFutureCrewAssignmentsForPersonnel,
  type PersonnelAssignmentDay,
} from "../scheduling/personnel-bridge.js";
...
import type { EmploymentType } from "./personnel.types.js";
import {
  assertEmail,
...
function eachDayInRange(start: string, end: string): string[] {
  ...
  if (a > b) throw new HttpError(400, "VALIDATION", "start must be <= end", "start");
  let guard = 0;
  for (let d = new Date(a); d <= b; d.setUTCDate(d.getUTCDate() + 1)) {
    out.push(d.toISOString().slice(0, 10));
    guard++;
    if (guard > 400) throw new HttpError(400, "VALIDATION", "Date range too large", "end");
  }
  return out;
}
...
export async function listPersonnel(
  ...
  const status = parseCsvEnums(query.status, ["active", "inactive", "on_leave"] as const);
  ...
  const sortBy = (
    ["name", "role", "department", "created_at"].includes(sortRaw)
      ? sortRaw
      : "name"
  ) as "name" | "role" | "department" | "created_at";
  const sortOrder = (query.sort_order ?? "asc") as "asc" | "desc";
  const filters: pRepo.PersonnelListFilters = {
    status,
  ...
  const rows = await pRepo.listPersonnel(
    pool,
    ctx.tenantId,
    filters,
    sortBy,
...
export async function createPersonnel(
  ...
  assertEmail(email);
  if (!first_name || !last_name) throw new HttpError(400, "VALIDATION", "Name required");
  const role = String(body.role ?? "");
  if (!role) throw new HttpError(400, "VALIDATION", "role required", "role");
  ...
  if (body.status !== undefined) {
    ...
    status = String(body.status) as PersonnelStatus;
  }
  ...
  if (department_id) {
    ...
    if (!d) throw new HttpError(400, "VALIDATION", "Department not found", "department_id");
  }
  const existing = await pRepo.findPersonnelByEmail(pool, ctx.tenantId, email);
  if (existing) throw new HttpError(409, "CONFLICT", "Email already exists", "email");
...
export async function updatePersonnel(
  ...
  if (!existing) throw new HttpError(404, "NOT_FOUND", "Personnel not found");

  const expectedVersion =
    body.version !== undefined && body.version !== null ? Number(body.version) : undefined;
  ...
  if (body.email !== undefined) {
    ...
    if (other && other.id !== id) throw new HttpError(409, "CONFLICT", "Email already exists", "email");
  }
  ...
  const updated = await pRepo.updatePersonnel(
    pool,
    ctx.tenantId,
    id,
    patch,
  ...
  const rateChanged =
    ("day_rate_amount" in patch &&
      (patch.day_rate_amount as number | null) !== (existing.day_rate ?? null)) ||
    ("per_diem_amount" in patch &&
      (patch.per_diem_amount as number | null) !== (existing.per_diem ?? null));
  if (rateChanged) {
    emitPersonnelRateChanged({
      personnel_id: id,
      tenant_id: ctx.tenantId,
...
export async function deletePersonnel(id: string): Promise<{ id: string; status: string; deactivated_at: string }> {
  ...
  if (!out) throw new HttpError(404, "NOT_FOUND", "Personnel not found");
...
async function availabilityCore(
  tenantId: string,
  personnelId: string,
  start: string,
  end: string,
  ...
  if (!p) throw new HttpError(404, "NOT_FOUND", "Personnel not found");
  ...
  const byDate = new Map<string, PersonnelAssignmentDay[]>();
  ...
  const outDays: AvailabilityDay[] = days.map((d) => {
    const blocked = isDateBlocked(d, blocks);
    const asgn = byDate.get(d) ?? [];
    const assignments = asgn.map((x: PersonnelAssignmentDay) => ({
...
export type BulkAvailabilityPersonnelRow = {
  personnel_id: string;
  first_name: string;
  last_name: string;
...
export async function listBulkAvailability(query: Record<string, string | undefined>): Promise<{
  ...
  const offset = decodeOffsetCursor(query.cursor);
  const statusParsed = parseCsvEnums(query.status, ["active", "inactive", "on_leave"] as const) ?? [];
  const filters: pRepo.PersonnelListFilters = {
  ...
  const blockRows = await pRepo.listBlockedDatesForPersonnelIdsInRange(
    pool,
    ctx.tenantId,
    ids,
    start,
  ...
  const blocksByPid = new Map<string, { start_date: string; end_date: string }[]>();
  ...
  const data: BulkAvailabilityPersonnelRow[] = rows.map((p) => {
...
export async function createInvitation(body: Record<string, unknown>): Promise<InvitationResponse> {
  const ctx = getContext();
  const email = String(body.email ?? "");
  assertEmail(email);
  const first_name = String(body.first_name ?? "");
...
export async function listInvitations(
  query: Record<string, string | undefined>,
): Promise<{ data: InvitationResponse[]; meta: { cursor: string | null; has_more: boolean; total_count: number } }> {
  const ctx = getContext();
  const limit = Math.min(
    100,
    Math.max(1, Number.parseInt(query.limit ?? "25", 10) || 25),
  ...
  const offset = decodeOffsetCursor(query.cursor);
  const status = query.status as InvitationResponse["status"] | undefined;
...
export async function revokeInvitation(id: string): Promise<{ id: string; status: string; revoked_at: string }> {
  const ctx = getContext();
  const out = await invRepo.revokeInvitation(pool, ctx.tenantId, id);
  if (!out) throw new HttpError(404, "NOT_FOUND", "Invitation not found or not pending");
  return { id: out.id, status: "revoked", revoked_at: out.revoked_at };
}

export async function acceptInvitation(
  token: string,
): Promise<{ personnel_id: string; invitation_id: string }> {
  const token_hash = invRepo.hashInvitationToken(token);
  const row = await invRepo.findInvitationByTokenHash(pool, token_hash);
  if (!row) throw new HttpError(400, "VALIDATION", "Invalid token");
  ...
  const id = randomUUID();
  const employment_type = (row.employment_type as string) || "freelance";
  assertEmploymentType(employment_type);
...
export async function getPersonnelById(
  personnel_id: string,
  tenant_id: string,
  options?: { include_deactivated?: boolean },
): Promise<PersonnelResponse | null> {
  const row = await pRepo.findPersonnelById(
    pool,
    tenant_id,
    personnel_id,
    options?.include_deactivated ?? false,
  );
  if (!row) return null;
...
export async function getPersonnelByDepartment(
  ...
  tenant_id: string,
  options?: { status?: PersonnelStatus[]; employment_type?: EmploymentType[] },
): Promise<PersonnelResponse[]> {
  const filters: pRepo.PersonnelListFilters = {
    department_id,
...
export async function listPersonnelBriefInternal(
  tenant_id: string,
  options?: { limit?: number },
): Promise<
  { id: string; first_name: string; last_name: string; role: string; email: string | null }[]
> {
...
export async function getAvailabilityInternal(
  personnel_id: string,
  tenant_id: string,
  start_date: string,
  end_date: string,
  ...
  personnel_id: string;
  days: { date: string; available: boolean; assignment_count: number }[];
}> {
...
export async function getDayRate(
  personnel_id: string,
  tenant_id: string,
  _options?: { event_id?: string; date?: string },
): Promise<{
  personnel_id: string;
  day_rate: number;
  currency: string;
  is_override: boolean;
  source: "default" | "event_override" | "date_effective";
}> {
...
export async function getPerDiem(
  personnel_id: string,
  tenant_id: string,
  _options?: { event_id?: string },
): Promise<{
  personnel_id: string;
  per_diem: number;
  currency: string;
