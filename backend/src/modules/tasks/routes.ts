...
function encodeCursor(t: TaskResponse): string {
  return Buffer.from(JSON.stringify({ u: t.updated_at, id: t.id }), "utf8").toString("base64url");
}

function decodeCursor(raw: string | undefined): { u: string; id: string } | null {
...
async function getPersonnelIdForUser(
  tenantId: string,
  userId: string,
): Promise<string | null> {
  const r = await pool.query<{ personnel_id: string | null }>(
    `SELECT personnel_id FROM users WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`,
    [tenantId, userId],
  );
  return r.rows[0]?.personnel_id ?? null;
}

async function assertPersonnel(tenantId: string, personnelId: string): Promise<boolean> {
  const r = await pool.query(
    `SELECT 1 FROM personnel WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`,
...
tasksRouter.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      let parentScope: "any" | "root" | string = "root";
      ...
      let minePid: string | null | undefined;
      if (q.mine) {
      ...
      const nextCursor =
        hasMore && slice.length > 0 ? encodeCursor(slice[slice.length - 1]) : null;
