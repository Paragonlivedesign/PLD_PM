...
export function analyticsDashboardRouter(pool: Pool): Router {
  const r = Router();

  r.get(
    "/operations",
    requireAnyPermission("analytics:dashboard:read", "events:read"),
  ...
  r.get(
    "/financial",
    requireAnyPermission("analytics:dashboard:read", "events:read"),
    requireAnyPermission("reports:read", "events:read"),
    asyncHandler(async (req, res) => {
      try {
