import type { ConflictAssignmentRef, ConflictSummary, CrewAssignmentResponse } from "./types.js";
...
import {
  calendarHoursBetweenIsoDates,
  driveHoursForDistanceKm,
  haversineKm,
} from "../../utils/geo.js";
...
import {
  getConflictById,
  insertSchedulingConflict,
  mapConflictRow,
  refreshHasConflictsForParticipants,
  resolveActiveDriveConflictsForPersonnel,
  resolveConflictsTouchingAssignment,
} from "./conflicts.repository.js";
import {
  findOverlappingCrewAssignments,
  getCrewAssignmentById,
  insertCrewAssignment,
  listAllActiveCrewAssignmentsForPersonnel,
  listCrewAssignments,
  listCrewAssignmentsByEventId,
  countCrewAssignments,
  mapCrewAssignmentRow,
  softDeleteCrewAssignment,
  updateCrewAssignment,
  type DbCrewAssignmentRow,
  type ListCrewAssignmentsParams,
} from "./crew-assignments.repository.js";

type Db = Pool | PoolClient;

function dbx(client?: Db): Db {
  return client ?? pool;
}
...
function personnelFullName(p: { first_name: string; last_name: string }): string {
  return `${p.first_name} ${p.last_name}`.trim();
}

function crewRef(row: DbCrewAssignmentRow): ConflictAssignmentRef {
  return {
...
function encodeCrewCursor(offset: number): string {
  return Buffer.from(JSON.stringify({ o: offset }), "utf8").toString("base64url");
}

export function decodeCrewCursor(cursor: string | null | undefined): number {
  if (!cursor) return 0;
  try {
...
async function ratesForPersonnel(
  tenantId: string,
  personnelId: string,
): Promise<{ dayRate: number; perDiem: number }> {
...
function summaryFromConflictRow(
  row: ReturnType<typeof mapConflictRow>,
  overlapDates: string,
): ConflictSummary {
  return {
    conflict_id: row.id,
...
async function createSoftConflictsForCrewPairwise(
  client: Db,
  tenantId: string,
  personnelId: string,
  ...
  for (const ex of overlapping) {
    const { start, end } = overlapRange(
      isoDate(newRow.start_date),
      isoDate(newRow.end_date),
      isoDate(ex.start_date),
      isoDate(ex.end_date),
    );
    const assignments: ConflictAssignmentRef[] = [crewRef(newRow), crewRef(ex)];
  ...
  await refreshHasConflictsForParticipants(client, tenantId, [
    ...
    ...overlapping.map((o) => ({ type: "crew" as const, id: o.id })),
  ]);
  return summaries;
}
 ...
 */
async function refreshDriveTimeConflictsForPersonnel(
  client: Db,
  tenantId: string,
  personnelId: string,
  personnelName: string,
  ...
  for (let i = 0; i < rows.length - 1; i++) {
    ...
    if (isoDate(first.end_date) >= isoDate(second.start_date)) continue;
    const gapH = calendarHoursBetweenIsoDates(
      isoDate(first.end_date),
      isoDate(second.start_date),
    );
    ...
    if (!ev1?.venue_id || !ev2?.venue_id) continue;
    ...
    if (full) {
      const mapped = mapConflictRow(full);
      summaries.push(
        summaryFromConflictRow(
          mapped,
          overlapDatesLabel(isoDate(first.end_date), isoDate(second.start_date)),
        ),
      );
      domainBus.emit("conflict.detected", {
        conflict_id: cid,
  ...
  if (summaries.length > 0) {
    await refreshHasConflictsForParticipants(
      client,
      tenantId,
      rows.map((r) => ({ type: "crew" as const, id: r.id })),
    );
  }
  return summaries;
}

function validateCrewStatus(s: string): s is CrewAssignmentResponse["status"] {
  return s === "tentative" || s === "confirmed" || s === "cancelled";
}

export async function createCrewAssignmentApi(
  input: {
    tenantId: string;
    userId: string;
    eventId: string;
  ...
  | { ok: true; data: CrewAssignmentResponse; conflicts: ConflictSummary[] }
...
> {
  ...
  const overlaps = await findOverlappingCrewAssignments(
    db,
    input.tenantId,
    input.personnelId,
    input.startDate,
  ...
  if (hasSoftOverlap) {
    conflicts = await createSoftConflictsForCrewPairwise(
      db,
      input.tenantId,
      input.personnelId,
      personnelFullName(personnel),
      row,
      overlaps,
    );
  }
  const driveCx = await refreshDriveTimeConflictsForPersonnel(
    db,
    input.tenantId,
    input.personnelId,
    personnelFullName(personnel),
...
export async function getCrewAssignment(
  ...
  id: string,
  client?: Db,
): Promise<CrewAssignmentResponse | null> {
...
export async function listCrewAssignmentsApi(
  p: Omit<ListCrewAssignmentsParams, "offset"> & { cursor: string | null },
): Promise<{
  rows: CrewAssignmentResponse[];
  total: number;
  ...
  const nextCursor =
    hasMore && slice.length > 0 ? encodeCrewCursor(offset + p.limit) : null;

  const rows: CrewAssignmentResponse[] = [];
...
export async function updateCrewAssignmentApi(
  tenantId: string,
  userId: string,
  id: string,
  patch: {
...
> {
  ...
  if (!cur) return { ok: false, status: 404, code: "NOT_FOUND", message: "Not found" };
  ...
  let departmentId =
    patch.departmentId !== undefined ? patch.departmentId : cur.department_id;
  let departmentName =
    patch.departmentName !== undefined ? patch.departmentName : cur.department_name;
  ...
  if (overlaps.length > 0) {
    conflicts = await createSoftConflictsForCrewPairwise(
      ...
      tenantId,
      cur.personnel_id,
      personnel ? personnelFullName(personnel) : cur.personnel_name,
      row,
      overlaps,
    );
  }
  ...
  domainBus.emit("assignment.updated", {
    ...
    changed_fields: Object.keys(patch).filter((k) => patch[k as keyof typeof patch] !== undefined),
    previous_values: {},
...
export async function deleteCrewAssignmentApi(
  tenantId: string,
  userId: string,
  id: string,
  client?: Db,
): Promise<{ ok: true; data: { id: string; deleted_at: string } } | { ok: false; status: 404 }> {
  const db = dbx(client);
  const cur = await getCrewAssignmentById(db, tenantId, id);
 ...
 */
export async function resyncCrewConflictsForEventAfterDateChange(
  tenantId: string,
  eventId: string,
  client?: Db,
): Promise<void> {
  ...
  for (const row of rows) {
    ...
    await createSoftConflictsForCrewPairwise(
      db,
      tenantId,
      row.personnel_id,
      row.personnel_name,
