import type { ConflictSummary, TruckAssignmentResponse } from "@pld/shared";
...
import {
  findOverlappingDriverAssignments,
  findOverlappingTruckAssignments,
  getTruckAssignmentById,
  insertTruckAssignment,
  listTruckAssignments,
  countTruckAssignments,
  mapTruckAssignmentRow,
  softDeleteTruckAssignment,
  updateTruckAssignment,
  type ListTruckAssignmentsParams,
...
export async function createTruckAssignmentApi(input: {
  ...
  | { ok: true; data: TruckAssignmentResponse; conflicts: ConflictSummary[] }
...
> {
  ...
  const overlaps = await findOverlappingTruckAssignments(
    db,
    input.tenantId,
    input.truckId,
    input.startDate,
  ...
  if (input.driverId) {
    const dOv = await findOverlappingDriverAssignments(
      db,
      input.tenantId,
      input.driverId,
      input.startDate,
...
export async function getTruckAssignment(
  tenantId: string,
  id: string,
  client?: Db,
): Promise<TruckAssignmentResponse | null> {
  const row = await getTruckAssignmentById(dbx(client), tenantId, id);
  return row ? mapTruckAssignmentRow(row) : null;
}

export async function listTruckAssignmentsApi(
  p: ListTruckAssignmentsParams,
): Promise<{
  rows: TruckAssignmentResponse[];
  total: number;
...
export async function updateTruckAssignmentApi(
  tenantId: string,
  userId: string,
  id: string,
  patch: {
...
> {
  ...
  const driverId =
    patch.driverId !== undefined ? patch.driverId : cur.driver_id;
...
export async function deleteTruckAssignmentApi(
  tenantId: string,
  userId: string,
  id: string,
  client?: Db,
