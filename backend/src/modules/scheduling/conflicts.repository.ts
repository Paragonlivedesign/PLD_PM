...
import type { ConflictAssignmentRef, ConflictResponse, SchedulingConflictKind } from "./types.js";

export type DbSchedulingConflictRow = {
  id: string;
  tenant_id: string;
  ...
  severity: string;
  status: string;
  overlap_start: Date | string;
  overlap_end: Date | string;
  detected_at: Date;
  resolved_at: Date | null;
  assignments: ConflictAssignmentRef[];
  event_id_1: string | null;
  event_id_2: string | null;
...
export function mapConflictRow(r: DbSchedulingConflictRow): ConflictResponse {
  const kind = (r.conflict_kind ?? "double_booking") as SchedulingConflictKind;
  return {
    id: r.id,
    resource_type: r.resource_type as ConflictResponse["resource_type"],
    resource_id: r.resource_id,
    resource_name: r.resource_name,
    severity: r.severity as ConflictResponse["severity"],
    status: r.status as ConflictResponse["status"],
...
export async function insertSchedulingConflict(
  client: Pool | PoolClient,
  p: {
    id: string;
    tenantId: string;
    resourceType: "personnel" | "truck";
    resourceId: string;
    resourceName: string;
    severity: "hard" | "soft";
    overlapStart: string;
  ...
  for (const a of p.assignments) {
    await client.query(
      `INSERT INTO scheduling_conflict_participants (conflict_id, assignment_type, assignment_id)
       VALUES ($1,$2,$3)`,
      [p.id, a.assignment_type, a.assignment_id],
    );
  }
  const full = await getConflictById(client, p.tenantId, p.id);
  if (!full) throw new Error("insertSchedulingConflict failed");
  return full;
}
...
export async function getConflictById(
  ...
  tenantId: string,
  id: string,
): Promise<DbSchedulingConflictRow | null> {
  const r = await client.query<DbSchedulingConflictRow & { assignments: unknown }>(
    `SELECT * FROM scheduling_conflicts WHERE tenant_id = $1 AND id = $2`,
    [tenantId, id],
  );
  const row = r.rows[0];
  if (!row) return null;
  const assignments =
    typeof row.assignments === "string"
      ? (JSON.parse(row.assignments) as ConflictAssignmentRef[])
      : (row.assignments as ConflictAssignmentRef[]);
...
export async function countConflicts(
  client: Pool | PoolClient,
  p: Omit<ListConflictsParams, "limit" | "cursorId">,
): Promise<number> {
...
export async function listConflicts(
  client: Pool | PoolClient,
  p: ListConflictsParams,
): Promise<DbSchedulingConflictRow[]> {
  const { where, vals } = buildConflictWhere(p);
  const cursorClause =
    p.cursorId != null ? ` AND c.id < $${vals.length + 1}::uuid` : "";
  const v = [...vals];
  if (p.cursorId) v.push(p.cursorId);
  v.push(p.limit + 1);
  const limIdx = v.length;
  const r = await client.query<DbSchedulingConflictRow & { assignments: unknown }>(
    `SELECT c.* FROM scheduling_conflicts c
     ${where}${cursorClause}
     ORDER BY c.id DESC
     LIMIT $${limIdx}`,
    v,
...
export async function refreshCrewHasConflicts(
  client: Pool | PoolClient,
  tenantId: string,
  crewAssignmentId: string,
): Promise<void> {
  await client.query(
    `UPDATE crew_assignments SET has_conflicts = EXISTS (
    ...
    [tenantId, crewAssignmentId, crewAssignmentId],
  );
}

export async function refreshTruckHasConflicts(
  client: Pool | PoolClient,
  tenantId: string,
  truckAssignmentId: string,
): Promise<void> {
  await client.query(
    `UPDATE truck_assignments SET has_conflicts = EXISTS (
       SELECT 1 FROM scheduling_conflict_participants p
       INNER JOIN scheduling_conflicts sc ON sc.id = p.conflict_id
     ...
     WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL`,
    [tenantId, truckAssignmentId, truckAssignmentId],
  );
}

export async function refreshHasConflictsForParticipants(
  client: Pool | PoolClient,
  tenantId: string,
  participants: { type: "crew" | "truck"; id: string }[],
): Promise<void> {
  ...
  for (const { type, id } of participants) {
    ...
    seen.add(key);
    if (type === "crew") await refreshCrewHasConflicts(client, tenantId, id);
    else await refreshTruckHasConflicts(client, tenantId, id);
  }
}

/** Clear drive-time rows before rebuilding adjacent-pair detection (same personnel). */
export async function resolveActiveDriveConflictsForPersonnel(
  client: Pool | PoolClient,
  tenantId: string,
  personnelId: string,
): Promise<void> {
  await client.query(
    `UPDATE scheduling_conflicts c SET status = 'resolved', resolved_at = NOW()
     WHERE c.tenant_id = $1 AND c.status = 'active' AND c.resource_type = 'personnel'
       AND c.resource_id = $2::uuid AND c.conflict_kind = 'drive_time_infeasible'`,
    [tenantId, personnelId],
  );
  const crew = await client.query<{ id: string }>(
    `SELECT id FROM crew_assignments
     WHERE tenant_id = $1 AND personnel_id = $2::uuid AND deleted_at IS NULL`,
    [tenantId, personnelId],
  );
  await refreshHasConflictsForParticipants(
    client,
    tenantId,
    crew.rows.map((x) => ({ type: "crew" as const, id: x.id })),
  );
}

export async function resolveConflictsTouchingAssignment(
  client: Pool | PoolClient,
  tenantId: string,
  assignmentType: "crew" | "truck",
  assignmentId: string,
): Promise<string[]> {
  const part = await client.query<{ assignment_type: string; assignment_id: string }>(
    `SELECT DISTINCT p.assignment_type, p.assignment_id
  ...
  const participants = part.rows.map((row) => ({
    type: row.assignment_type as "crew" | "truck",
...
export async function countActiveConflictsForTenant(client: Pool | PoolClient, tenantId: string): Promise<number> {
  const r = await client.query<{ c: number }>(
    `SELECT count(*)::int AS c FROM scheduling_conflicts WHERE tenant_id = $1 AND status = 'active'`,
    [tenantId],
  );
  return r.rows[0]?.c ?? 0;
}
