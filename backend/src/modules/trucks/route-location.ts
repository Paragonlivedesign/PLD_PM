...
import type { RouteLocationRef } from "@pld/shared";
import { getContactById } from "../contacts/repository.js";
import { getVenueById } from "../venues/repository.js";

export function parseRouteLocationRef(raw: unknown): RouteLocationRef | null {
...
export function refToJson(ref: RouteLocationRef | null): Record<string, unknown> {
  if (!ref) return {};
  if (ref.kind === "address") return { kind: "address", text: ref.text };
  ...
  if (ref.kind === "venue") return { kind: "venue", venue_id: ref.venue_id };
  return {
    kind: "contact",
...
function contactAddressLabel(c: Awaited<ReturnType<typeof getContactById>>, custom?: string | null): string {
  if (!c) return "Contact";
  if (custom && custom.trim()) return `${c.name} — ${custom.trim()}`;
  ...
  const addr =
    typeof meta.address === "string"
      ...
      : typeof meta.street === "string"
        ? meta.street
        : "";
  if (addr) return `${c.name} — ${addr}`;
  return c.name;
}

export async function resolveRouteLocationLabel(
  client: Pool | PoolClient,
  tenantId: string,
  ref: RouteLocationRef,
): Promise<string> {
  if (ref.kind === "address") return ref.text;
  if (ref.kind === "coordinates") {
...
export type ResolvedCoordinate = { lat: number; lng: number; label: string };

/** Best-effort coordinate for directions (venue lat/lng, coordinates, geocode not implemented). */
export async function resolveRouteLocationCoordinate(
  client: Pool | PoolClient,
  tenantId: string,
  ref: RouteLocationRef,
