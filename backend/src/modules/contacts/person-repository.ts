...
import type { ContactPersonResponse, ContactParentType } from "@pld/shared";
import {
  emailsFromSingle,
  parseEmailsJson,
  parsePhonesJson,
  phonesFromSingle,
  primaryEmailNormalizedFromEmails,
} from "./person-utils.js";
...
function mapPerson(r: PersonRow): ContactPersonResponse {
...
export async function insertContactPerson(
  ...
  const r = await client.query<PersonRow>(
...
export async function getContactPersonById(
  client: Pool | PoolClient,
  tenantId: string,
  id: string,
): Promise<ContactPersonResponse | null> {
  const r = await client.query<PersonRow>(
...
export async function syncPersonFromContactFields(
  client: Pool | PoolClient,
  tenantId: string,
  personId: string,
  displayName: string,
...
export type HubPersonRow = ContactPersonResponse & {
  memberships: HubMembershipRow[];
};

export async function listContactPersonsForHub(
  client: Pool | PoolClient,
  tenantId: string,
  opts: {
    limit: number;
    search?: string | null;
    parentType?: ContactParentType | null;
    parentId?: string | null;
  },
): Promise<HubPersonRow[]> {
  ...
  const r = await client.query<PersonRow & { memberships: unknown }>(
    `WITH ids AS (
  ...
  return r.rows.map((row) => {
    ...
    const memberships: HubMembershipRow[] = arr.map((x) => {
      const o = x as Record<string, unknown>;
      return {
        membership_id: String(o.membership_id ?? ""),
        parent_type: o.parent_type as ContactParentType,
    ...
    return { ...mapPerson(rest as PersonRow), memberships };
  });
}
