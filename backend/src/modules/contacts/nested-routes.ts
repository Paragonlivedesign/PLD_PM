...
import {
  enrichContactWithPersonnel,
  enrichContactsWithPersonnel,
} from "./enrich-personnel.js";
...
export function createContactsNestedRouter(config: {
  ...
  r.get(
    "/",
    requirePermission(config.readPerm),
    asyncHandler(async (req, res) => {
      ...
      const rows = await contactsRepo.listContactsForParent(
        pool,
        req.ctx.tenantId,
        config.parentType,
        parentId,
  ...
  r.post(
    "/",
    requirePermission(config.writePerm),
    asyncHandler(async (req, res) => {
      const parentId = routeParam(req.params[config.paramKey]);
      const chk = await assertParent(req.ctx.tenantId, config.parentType, parentId);
      if (!chk.ok) {
      ...
      try {
        ...
        try {
          ...
          res.status(201).json(
            ok(await enrichContactWithPersonnel(req.ctx.tenantId, row)),
          );
        } catch (e) {
          try {
  ...
  r.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const row = await contactsRepo.getContactById(
        pool,
        req.ctx.tenantId,
  ...
  r.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      const existing = await contactsRepo.getContactById(
        pool,
        req.ctx.tenantId,
        routeParam(req.params.contactId),
      );
      ...
      try {
        ...
        const updated = await contactsRepo.updateContactRow(
          pool,
          req.ctx.tenantId,
          routeParam(req.params.contactId),
          {
  ...
  r.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const del = await contactsRepo.softDeleteContact(
        pool,
        req.ctx.tenantId,
        routeParam(req.params.contactId),
        config.parentType,
