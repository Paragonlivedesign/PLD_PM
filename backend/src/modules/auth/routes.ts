...
authPublicRouter.post(
  "/refresh",
  asyncHandler(async (req, res) => {
    const data = await svc.refresh(req.body as { refresh_token?: string });
    res.status(200).json(ok(data));
  }),
);
...
authPublicRouter.post(
  "/forgot-password",
  asyncHandler(async (req, res) => {
    const data = await svc.forgotPassword(req.body as Record<string, string>);
    res.status(200).json(ok(data));
  }),
);
...
authPublicRouter.post(
  "/reset-password",
  asyncHandler(async (req, res) => {
    const data = await svc.resetPassword(req.body as Record<string, string>);
    res.status(200).json(ok(data));
  }),
);
...
authPublicRouter.post(
  "/invitations/accept",
  asyncHandler(async (req, res) => {
    const data = await svc.acceptInvitation(req.body as Record<string, string>);
    res.status(200).json(ok(data));
  }),
);
...
authProtectedRouter.post(
  "/logout",
  asyncHandler(async (req, res) => {
    const data = await svc.logout(req.body as { refresh_token?: string });
    res.status(200).json(ok(data));
  }),
);
...
authProtectedRouter.post(
  ...
  requirePermission("auth.sessions.revoke"),
  asyncHandler(async (req, res) => {
    const ctx = getContext();
...
authProtectedRouter.put(
  "/me",
  asyncHandler(async (req, res) => {
    const ctx = getContext();
    const data = await svc.updateMe(ctx.tenantId, ctx.userId, req.body as Record<string, unknown>);
    res.status(200).json(ok(data));
  }),
);
...
authProtectedRouter.post(
  "/invite",
  requirePermission("auth.invitations.manage"),
  asyncHandler(async (req, res) => {
    const ctx = getContext();
    const data = await svc.inviteUser(ctx.tenantId, ctx.userId, req.body as Record<string, unknown>);
    res.status(201).json(ok(data));
  }),
);
