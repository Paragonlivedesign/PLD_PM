...
function openAllNotifications() {
  openModal(
    'Notifications',
    `<p style="margin:0;font-size:13px;color:var(--text-tertiary);">Notifications are loaded from the API when configured (see <code style="font-size:11px;">pld-notifications-api.js</code>). No demo feed.</p>`,
    `<button class="btn btn-primary" onclick="closeModal()">Close</button>`,
  );
}

function submitCloneEventForm() {
  const srcEl = document.getElementById('cloneEventSource');
  const nameEl = document.getElementById('cloneEventName');
  const startEl = document.getElementById('cloneEventStart');
  const endEl = document.getElementById('cloneEventEnd');
  const incVenue = document.getElementById('cloneOptVenue');
  const incTags = document.getElementById('cloneOptTags');
  const incMeta = document.getElementById('cloneOptMeta');
  const sourceId = srcEl && srcEl.value ? srcEl.value : '';
  const name = nameEl && nameEl.value ? nameEl.value.trim() : '';
  const start_date = startEl && startEl.value ? startEl.value : '';
  const end_date = endEl && endEl.value ? endEl.value : '';
  if (!sourceId || !name || !start_date || !end_date) {
    showToast('Source, name, and dates are required', 'error');
    return;
  }
  if (typeof PLD_EVENTS_FROM_REST !== 'undefined' && PLD_EVENTS_FROM_REST) {
    void (async () => {
      const body = {
        ...
        end_date: end_date,
        clone_options: {
          include_venue: incVenue ? incVenue.checked : true,
          include_tags: incTags ? incTags.checked : true,
          include_metadata: incMeta ? incMeta.checked : true,
        },
      };
      const created = await pldCloneEventViaApi(sourceId, body);
      if (created) {
  ...
  showToast('Event cloned! (demo — enable API for real clone)', 'success');
  closeModal();
}
  ...
  openModal('Clone Event', `
    <p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;">${
      typeof PLD_EVENTS_FROM_REST !== 'undefined' && PLD_EVENTS_FROM_REST
        ? 'Creates a new event via the API (crew, travel, and documents are not copied — per contract).'
        : 'Clone an event with all its details, crew, and truck assignments.'
    }</p>
    <div class="form-group"><label class="form-label">Source Event</label><select class="form-select" id="cloneEventSource">${EVENTS.map((e) => `<option value="${e.id}">${e.name} (${phaseLab(e.phase)})</option>`).join('')}</select></div>
    <div class="form-group"><label class="form-label">New Event Name</label><input type="text" class="form-input" id="cloneEventName" placeholder="e.g. Super Bowl LXII Pre-Show"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      <div class="form-group"><label class="form-label">New Start Date</label><input type="date" class="form-input" id="cloneEventStart"></div>
      <div class="form-group"><label class="form-label">New End Date</label><input type="date" class="form-input" id="cloneEventEnd"></div>
    </div>
    <div class="form-group"><label class="form-label">API clone options</label>
      <div style="display:flex;flex-direction:column;gap:8px;">
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;"><input type="checkbox" id="cloneOptVenue" checked> Include venue</label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;"><input type="checkbox" id="cloneOptTags" checked> Include tags</label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;"><input type="checkbox" id="cloneOptMeta" checked> Include metadata (and custom fields when included)</label>
      </div>
    </div>
...
function openPersonnelCSVImportModal() {
  if (typeof window.pldPersonnelCsvOpenWizard === 'function' && typeof window.pldApiFetch === 'function') {
    window.pldPersonnelCsvOpenWizard();
    return;
  }
  csvImportStep = 1;
...
function openAssignTruckModal(eventId) {
  const ev = EVENTS.find(e => e.id === eventId);
  const assignedT = Array.isArray(ev.trucks) ? ev.trucks : [];
  const available = TRUCKS.filter(t => !assignedT.includes(t.id));
  openModal('Assign Truck — ' + ev.name, `
    <input type="hidden" id="assignTruckEventId" value="${String(eventId).replace(/"/g, '&quot;')}">
    ${available.map(t => `<div style="display:flex;align-items:center;gap:10px;padding:10px;border-bottom:1px solid var(--border-subtle);cursor:pointer;" onclick="this.querySelector('input').checked=!this.querySelector('input').checked"><input type="checkbox" class="assign-truck-cb" value="${String(t.id).replace(/"/g, '&quot;')}" onclick="event.stopPropagation()"><div style="font-size:20px;">🚛</div><div style="flex:1;"><div style="font-weight:500;font-size:13px;">${t.name}</div><div style="font-size:11px;color:var(--text-tertiary);">${t.type} · ${t.location}</div></div><span class="phase-badge" style="font-size:10px;">${t.status}</span></div>`).join('')}
    <div class="form-group" style="margin-top:16px;"><label class="form-label">Route origin (purpose)</label><input type="text" id="assignTruckPurpose" class="form-input" placeholder="Warehouse / previous venue"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      <div class="form-group"><label class="form-label">Start date</label><input type="date" id="assignTruckStart" class="form-input" value="${ev.startDate || ''}"></div>
      <div class="form-group"><label class="form-label">End date</label><input type="date" id="assignTruckEnd" class="form-input" value="${ev.endDate || ev.startDate || ''}"></div>
    </div>
    <div class="form-group"><label class="form-label">Notes</label><textarea id="assignTruckNotes" class="form-textarea" placeholder="Stops, special instructions…"></textarea></div>
...
function openAssignTruckToEventModal(truckId) {
  ...
  if (!t) {
    if (typeof showToast === 'function') showToast('Truck not found.', 'error');
    return;
  }
...
function openCreateAssignmentModal(personId, dateStr) {
  ...
  openModal('Create Assignment', `
    ...
    <div class="form-group"><label class="form-label">Event</label><select class="form-select"><option value="">Select event…</option>${EVENTS.filter(e => !isTerminalEventPhase(e.phase)).map(e => `<option>${e.name}</option>`).join('')}</select></div>
...
function openCreateInvoiceModal() {
  openModal('New Invoice', `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      ...
      <div class="form-group"><label class="form-label">Event</label><select class="form-select"><option value="">Select…</option>${EVENTS.filter(e => !isTerminalEventPhase(e.phase)).map(e => `<option value="${e.id}">${e.name}</option>`).join('')}</select></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
...
function openUploadDocModal() {
  ...
  openModal('Upload Document', `
    <div class="form-group"><label class="form-label">File</label><input type="file" id="pldUploadDocFile" class="form-input"></div>
    <div class="form-group"><label class="form-label">Event</label><select class="form-select" id="pldUploadDocEvent"><option value="">Optional</option>${eventOpts}</select></div>
    <div class="form-group"><label class="form-label">Category</label><select class="form-select" id="pldUploadDocCategory">
    ...
    <div class="form-group"><label class="form-label">Name</label><input type="text" class="form-input" id="pldUploadDocName" placeholder="Defaults to filename"></div>
    <div class="form-group"><label class="form-label">Description</label><textarea class="form-textarea" id="pldUploadDocDesc" placeholder="Optional"></textarea></div>
    <p id="pldUploadDocErr" style="font-size:12px;color:var(--accent-red);display:none;"></p>
  `, `<button class="btn btn-secondary" onclick="closeModal()">Cancel</button><button class="btn btn-primary" id="pldUploadDocBtn" onclick="pldSubmitDocumentUpload()">Upload</button>`);
}
...
function openEditUserModal(name, email, role) {
  ...
  const n = pldModalEscAttr(name);
  const e = pldModalEscAttr(email);
  const body = `
    <div class="form-group"><label class="form-label">Name</label><input type="text" class="form-input" value="${n}"></div>
    <div class="form-group"><label class="form-label">Email</label><input type="email" class="form-input" value="${e}"></div>
    <div class="form-group"><label class="form-label">Role</label><select class="form-select">${roles.map(r => `<option ${r === role ? 'selected' : ''}>${pldModalEscAttr(r)}</option>`).join('')}</select></div>
  ...
  `;
  const saveOnclick =
    'showToast(' + JSON.stringify(String(name) + ' updated!') + ',' + JSON.stringify('success') + ');closeModal();';
  const footer =
    `<button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>` +
    `<button type="button" class="btn btn-primary" onclick="${pldModalEscAttr(saveOnclick)}">Save</button>`;
  openModal('Edit: ' + name, body, footer);
}
...
function openRefreshModal() {
  const spinStyle =
    '<style>@keyframes pldRefreshSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}</style>';
  const loadingBody = `
    <div style="text-align:center;padding:20px 0;">
      ${spinStyle}
      <div style="font-size:40px;margin-bottom:16px;animation:pldRefreshSpin 1s linear infinite;">⟳</div>
      <div style="font-size:14px;font-weight:500;margin-bottom:8px;">Refreshing catalog…</div>
      <div style="font-size:12px;color:var(--text-tertiary);max-width:360px;margin:0 auto 20px;line-height:1.5;">Calling <code style="font-size:10px;">pldHydrateCatalogFromRest</code> (PostgreSQL API). This replaces in-memory lists when the server returns data.</div>
    </div>
  `;
  openModal(
    'Refresh Data',
    loadingBody,
    `<button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>` +
      `<button type="button" class="btn btn-primary" id="pldRefreshDoneBtn" disabled>Working…</button>`,
  );
  ...
  void (async function () {
    ...
    let errMsg = '';
    try {
      if (typeof window.pldHydrateCatalogFromRest === 'function' && typeof window.pldApiFetch === 'function') {
        ok = await window.pldHydrateCatalogFromRest();
        if (!ok) errMsg = 'API health check failed or server did not return catalog data.';
      } else if (typeof window.pldApiFetch !== 'function') {
        errMsg = 'REST client not loaded.';
      } else {
        errMsg = 'Catalog hydrate helper not available.';
      }
    } catch (e) {
    ...
    const doneBtn = document.getElementById('pldRefreshDoneBtn');
    if (!bodyEl) return;

    const ev = typeof EVENTS !== 'undefined' ? EVENTS.length : 0;
    const pe = typeof PERSONNEL !== 'undefined' ? PERSONNEL.length : 0;
    const tr = typeof TRUCKS !== 'undefined' ? TRUCKS.length : 0;
    const tv = typeof TRAVEL_RECORDS !== 'undefined' ? TRAVEL_RECORDS.length : 0;
    const dc = typeof DOCUMENTS !== 'undefined' ? DOCUMENTS.length : 0;

    const success = ok && !errMsg;
    const icon = success ? '✓' : '⚠';
    const iconColor = success ? 'var(--accent-green)' : 'var(--accent-amber)';
    const headline = success ? 'Catalog refreshed' : 'Refresh incomplete';
    const sub = success
      ? 'Lists were reloaded from the API (per module: events, personnel, trucks, travel, documents, etc.).'
      : pldModalEscAttr(errMsg);

    bodyEl.innerHTML = `
      <div style="text-align:center;padding:12px 0 8px;">
        <div style="font-size:36px;margin-bottom:12px;color:${iconColor};">${icon}</div>
        <div style="font-size:15px;font-weight:600;margin-bottom:6px;">${headline}</div>
        <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:16px;line-height:1.5;max-width:400px;margin-left:auto;margin-right:auto;">${sub}</div>
        <div style="text-align:left;background:var(--bg-tertiary);padding:16px;font-family:monospace;font-size:11px;max-height:220px;overflow-y:auto;border-radius:8px;">
          <div style="color:var(--text-secondary);margin-bottom:8px;font-weight:600;">In-memory counts now</div>
          <div style="margin-bottom:3px;">Events: ${ev}</div>
          <div style="margin-bottom:3px;">Personnel: ${pe}</div>
          <div style="margin-bottom:3px;">Trucks: ${tr}</div>
          <div style="margin-bottom:3px;">Travel: ${tv}</div>
          <div style="margin-bottom:3px;">Documents: ${dc}</div>
        </div>
      </div>
    `;
    ...
    if (doneBtn) {
      ...
      doneBtn.onclick = function () {
        if (success && typeof showToast === 'function') showToast('Data refreshed', 'success');
        closeModal();
...
function openAutoOptimizeModal() {
  ...
  const activeEvts = EVENTS.filter(e => !isTerminalEventPhase(e.phase)).length;
...
function openCompanyInviteMemberModal() {
  openModal('Invite Team Member', `
    ...
    <div class="form-group"><label class="form-label">Email</label><input type="email" class="form-input" placeholder="user@example.com"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
