...
function switchEventTab(tab) {
  ...
  if (tab === 'travel' && selectedEventId && typeof pldRefreshEventTravel === 'function') {
    void pldRefreshEventTravel(selectedEventId);
    return;
  }
  renderPage('event');
  if (tab === 'crew' && selectedEventId && typeof pldRefreshEventCrewAssignments === 'function') {
    setTimeout(function () {
      void pldRefreshEventCrewAssignments(selectedEventId);
    }, 0);
  }
  if (tab === 'trucks' && selectedEventId && typeof pldRefreshEventTruckAssignments === 'function') {
    setTimeout(function () {
      void pldRefreshEventTruckAssignments(selectedEventId);
    }, 0);
  }
}

async function saveEventHeaderField(eventId, field, value) {
...
function renderEventPage() {
  ...
  const tabs = [
    ...
    { key: 'crew', label: `Crew (${crewN})` },
    { key: 'trucks', label: `Trucks (${trucksN})` },
  ...
  return `
    <div class="ep-overview-grid">
      ...
      <div class="ep-stats-row">
        ...
        <div class="stat-card"><div class="stat-label">Crew</div><div class="stat-value">${crew.length}</div></div>
        <div class="stat-card"><div class="stat-label">Trucks</div><div class="stat-value">${trucks.length}</div></div>
      ...
      <div class="ep-two-col">
        ...
        <div class="ep-section">
          ...
          ${crew.length > 0 ? crew.slice(0, 5).map(cid => {
            const p = getPersonnel(cid);
            if (!p) return '';
            const dept = getDepartment(p.dept);
            return `<div class="ep-list-item" onclick="openPersonnelDetail('${p.id}')"><div class="ep-avatar" style="background:${p.avatar};">${p.initials}</div><div class="ep-list-info"><div class="ep-list-name">${p.name}</div><div class="ep-list-sub">${p.role} · ${dept ? dept.name : '—'}</div></div><div class="ep-list-extra">${formatCurrency(p.rate)}/day</div></div>`;
          }).join('') + (crew.length > 5 ? `<div class="ep-list-more" onclick="switchEventTab('crew')">+${crew.length - 5} more</div>` : '') : '<div class="ep-empty">No crew assigned</div>'}
        </div>
        ...
        <div class="ep-section">
          ...
          ${trucks.length > 0 ? trucks.map(tid => {
      ...
      <div class="ep-section">
        <div class="ep-section-header">Primary contact</div>
        <p class="form-hint" style="margin:0 0 8px;font-size:12px;">CRM contact from the client or venue directory.</p>
        <select id="pldEpPrimaryContactSelect" class="form-select" style="max-width:480px;" data-event-id="${ev.id}"
          onchange="void pldSaveEventPrimaryContactFromSelect(this)">
          <option value="">Loading…</option>
        </select>
      </div>
      <div class="ep-section">
        <div class="ep-section-header">Custom fields</div>
        <div id="epCustomFieldsMount"></div>
        <button type="button" class="btn btn-secondary btn-sm" style="margin-top:8px;" onclick="pldSaveOverviewCustomFields('${ev.id}')">Save custom fields</button>
      </div>` : ''}
...
function renderEventCrewTab(ev, client, venue) {
  const crewIds = Array.isArray(ev.crew) ? ev.crew : [];
  const assignedCrew = crewIds.map(cid => getPersonnel(cid)).filter(Boolean);
  const deptGroups = {};
  assignedCrew.forEach(p => {
    const dept = getDepartment(p.dept);
    const dn = dept ? dept.name : '—';
    if (!deptGroups[dn]) deptGroups[dn] = { color: dept ? dept.color : '#64748b', members: [] };
    deptGroups[dn].members.push(p);
  });

  return `
    <div class="ep-crew-tab">
      <div id="pldEventAssignmentsPanel" class="pld-event-assignments-api" style="margin-bottom:16px;"></div>
      <div class="ep-tab-header">
        <div class="ep-tab-header-left">
          <h3>${crewIds.length} Crew Members</h3>
      ...
      ${crewIds.length === 0 ? '<div class="ep-empty-state"><h3>No crew assigned</h3><p>Add crew members to get started.</p><button class="btn btn-primary" onclick="openAssignCrewModal(\'' + ev.id + '\')">+ Assign Crew</button></div>' : ''}
    </div>
  `;
}
...
function renderEventTravelTab(ev, client, venue) {
  const apiCache = typeof window !== 'undefined' && window.__pldEventTravelByEventId
    ? window.__pldEventTravelByEventId[ev.id]
    : null;
  if (apiCache && apiCache.error) {
    return `
      <div class="ep-travel-tab">
        <div class="ep-tab-header"><h3>Travel</h3></div>
        <p style="color:var(--accent-amber);font-size:13px;">Could not load travel from API (HTTP ${apiCache.status ?? '—'}). Check backend and database migration 003.</p>
        <button type="button" class="btn btn-secondary btn-sm" onclick="pldRefreshEventTravel('${ev.id}')">Retry</button>
      </div>`;
  }
  if (apiCache && apiCache.data && Array.isArray(apiCache.data.personnel_travel)) {
    const meta = apiCache.meta || {};
    const rows = [];
    let flights = 0;
    let withHotel = 0;
    for (const g of apiCache.data.personnel_travel) {
      for (const rec of g.records || []) {
        rows.push({ rec, personnel_name: g.personnel_name });
        if (rec.travel_type === 'flight') flights++;
        if (rec.accommodation) withHotel++;
      }
    }
    const totalTravel = Number(meta.total_travel_cost || 0);
    const totalAcc = Number(meta.total_accommodation_cost || 0);
    const totalCost = totalTravel + totalAcc;
    const apiLabel = ' · API';
    return `
    <div class="ep-travel-tab">
      <div class="ep-tab-header">
        <div class="ep-tab-header-left">
          <h3>${rows.length} Travel Records${apiLabel}</h3>
          <span style="color:var(--text-tertiary);font-size:13px;">Travel: ${formatCurrency(totalTravel)} · Hotels: ${formatCurrency(totalAcc)} · ${meta.currency || 'USD'}</span>
        </div>
        <div class="ep-tab-header-right">
          <button class="btn btn-ghost btn-sm" onclick="pldRefreshEventTravel('${ev.id}')">Refresh</button>
      ...
      <div class="ep-stats-row" style="margin-bottom:16px;">
        <div class="stat-card"><div class="stat-label">Flights</div><div class="stat-value">${flights}</div></div>
        <div class="stat-card"><div class="stat-label">With hotel</div><div class="stat-value">${withHotel}</div></div>
        <div class="stat-card"><div class="stat-label">People</div><div class="stat-value">${meta.total_personnel ?? '—'}</div></div>
      ...
      ${rows.length > 0 ? `
        <div class="table-wrap">
          <table class="data-table">
            <thead><tr><th>Type</th><th>Person</th><th>Route</th><th>Departs</th><th>Cost</th><th>Status</th></tr></thead>
            <tbody>
              ${rows.map(({ rec, personnel_name }) => {
                const travelIconKey = rec.travel_type === 'flight' ? 'travelFlight' : (rec.accommodation ? 'travelHotel' : 'travelLocation');
                const dep = rec.departure_datetime ? rec.departure_datetime.slice(0, 16).replace('T', ' ') : '—';
                const leg = (rec.departure_location || '') + ' → ' + (rec.arrival_location || '');
                const c = (Number(rec.cost || 0) + Number(rec.accommodation && rec.accommodation.total_cost ? rec.accommodation.total_cost : 0));
                return `<tr title="API record ${rec.id}">
                  <td>${uiIcon(travelIconKey)} ${String(rec.travel_type || '').replace(/_/g, ' ')}</td>
                  <td><span class="ep-clickable">${personnel_name || '—'}</span></td>
                  <td>${leg}${rec.carrier ? ` · ${rec.carrier}` : ''}</td>
                  <td>${dep}</td>
                  <td>${formatCurrency(c)}</td>
                  <td><span class="phase-badge" style="font-size:10px;">${rec.status}</span></td>
                </tr>`;
              }).join('')}
            </tbody>
          </table>
        </div>
      ` : '<div class="ep-empty-state"><h3>No travel in API</h3><p>Create records via POST /api/v1/travel or use demo data without API base.</p></div>'}
    </div>
  `;
  }
  ...
  const travelDemoHint =
    typeof PLD_EVENTS_FROM_REST !== 'undefined' && PLD_EVENTS_FROM_REST
      ? '<p style="font-size:12px;color:var(--text-tertiary);margin:0 0 14px 0;">When the travel API returns data for this event, it replaces this view. Until then, only <strong>demo</strong> rows linked to seed event IDs may appear.</p>'
      : '';
...
function renderEventFinancialTab(ev, client, venue) {
  ...
  const financialHint =
    ev.budget <= 0 && ev.spent <= 0
      ? '<p style="font-size:12px;color:var(--text-tertiary);margin:0 0 14px 0;">Set an event <strong>budget</strong> from the header (or overview) to anchor this summary. Category split below is illustrative until wired to live financial lines from the API.</p>'
      : '<p style="font-size:12px;color:var(--text-tertiary);margin:0 0 14px 0;">Category breakdown is <strong>illustrative</strong> (derived from spent total). Detailed P&amp;L will use financial module data when connected.</p>';
...
function renderEventDocumentsTab(ev, client, venue) {
  ...
  const docHint =
    typeof PLD_EVENTS_FROM_REST !== 'undefined' && PLD_EVENTS_FROM_REST
      ? '<p style="font-size:12px;color:var(--text-tertiary);margin:0 0 14px 0;">This list uses local <strong>DOCUMENTS</strong> seed data keyed to demo event IDs. Hook <code>GET /api/v1/documents</code> (or equivalent) here when ready for production files.</p>'
      : '';
...
window.pldPopulateEventPrimaryContactSelect = async function (eventId) {
  const sel = document.getElementById('pldEpPrimaryContactSelect');
  if (!sel || typeof pldListContactsForParent !== 'function') return;
  const ev = EVENTS.find(function (e) {
    return e.id === eventId;
  });
  if (!ev) return;
  const cur = ev.primary_contact_id ? String(ev.primary_contact_id) : '';
  let clientRows = [];
  let venueRows = [];
  if (ev.client) {
    clientRows = await pldListContactsForParent('client', String(ev.client));
  }
  if (ev.venue) {
    venueRows = await pldListContactsForParent('venue', String(ev.venue));
  }
...
window.pldSaveEventPrimaryContactFromSelect = function (sel) {
  const eventId = sel.getAttribute('data-event-id');
  const v = sel.value || '';
  void (async function () {
    await persistEventFields(eventId, { primary_contact_id: v || null });
    if (typeof showToast === 'function') showToast('Primary contact saved', 'success');
  })();
};
