  ...
  return `
    ...
    <div class="schedule-controls">
      <div class="view-toggle">
        ...
        <button class="view-toggle-btn ${scheduleView === 'api' ? 'active' : ''}" onclick="scheduleView='api'; renderPage('scheduling');" title="Crew rows from Postgres /api/v1/schedule">Live sheet</button>
      </div>
    ...
    <div class="card" style="padding:16px;margin-top:8px;">
      <p style="font-size:13px;color:var(--text-secondary);margin:0 0 12px;">Weekly crew assignments from <code>/api/v1/schedule</code> (requires API base + migrated DB).</p>
      <div id="pldSchedulingApiPanel"><p style="font-size:13px;color:var(--text-tertiary);">Loading…</p></div>
    </div>
...
function renderScheduleTimeline(opts) {
  ...
  const activeEvents = EVENTS.filter(e => !isTerminalEventPhase(e.phase));
  ...
  const phaseColors = {
    ...
    pre_production: 'var(--accent-purple)',
    production: 'var(--accent-blue)',
    post_production: 'var(--accent-amber)',
...
function initScheduleTimelineBarDnD() {
  ...
  function commitMove(eventId, newStart, newEnd) {
    ...
    ev.endDate = newEnd;
    persistEventFields(eventId, { startDate: newStart, endDate: newEnd });
    ...
    if (ev) {
      if (dragging.type === 'move') {
        ...
        if (newStart < origEndDate) {
          ev.startDate = newStart;
          persistEventFields(ev.id, { startDate: newStart });
          renderPage('scheduling');
        ...
        if (newEnd > origStartDate) {
          ev.endDate = newEnd;
          persistEventFields(ev.id, { endDate: newEnd });
          renderPage('scheduling');
...
function renderScheduleCalendar(opts) {
  ...
  function eventsOnDay(day) {
    ...
    return EVENTS.filter(ev => {
      return !isTerminalEventPhase(ev.phase) && ev.startDate <= dateStr && ev.endDate >= dateStr;
    });
  }
